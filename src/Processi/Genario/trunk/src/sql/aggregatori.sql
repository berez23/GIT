...
SQL_LEGGI_SOGGETTO_TOTALE_FONTE_RIF=SELECT  id_dwh iddwh,  fk_ente_sorgente enteSorgente, ctr_hash ctrHash , cognome, nome, denominazione, sesso, codfisc codfis \
							...
							AND RATING is null AND nvl(FK_SOGGETTO,0) = '0' AND REL_DESCR is null AND ANOMALIA is null and flag_runtime_non_agganciato ='0'
...
SQL_INSERISCI_SOGGETTO_UNICO=INSERT INTO SIT_SOGGETTO_UNICO (CTRL_UTIL, COGNOME, NOME, DENOMINAZIONE,\
                                ...
                                DESC_PROVINCIA_RES,DESC_COMUNE_RES,DT_INS,ID_SOGGETTO,RATING,CODICE_SOGGETTO,VALIDATO,FONTE_RIF) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,sysdate,?,?,?,?,?)
...
SQL_UPDATE_SOGGETTO_TOTALE_FK_ALL=update sit_soggetto_totale tt set fk_soggetto = ?\
                                WHERE FK_SOGGETTO = ?\
                                and not exists \
                                (select 1 from SIT_SOGGETTO_UNICO U \
                                WHERE (U.FONTE_RIF = '1' OR VALIDATO ='1') \
                                AND ID_SOGGETTO = ?)
/*queste due sql servono per far rimanere sempre soggetto di riferimento quello giusto anche quando vado a cambiare massivamente gli fk correlati (vedi codice)*/
/*
 * NON USATE
 */
SQL_RIPRISTINA_FK_SOGGETTO_RIF1=UPDATE sit_soggetto_unico U set ID_SOGGETTO =? where U.ID_SOGGETTO=? AND U.FONTE_RIF=1
SQL_RIPRISTINA_FK_SOGGETTO_RIF2=UPDATE sit_soggetto_unico set ID_SOGGETTO =? where ID_SOGGETTO=? AND FONTE_RIF=0
SQL_RIPRISTINA_FK_SOGGETTO_RIF3=UPDATE sit_soggetto_unico U set ID_SOGGETTO =? where U.ID_SOGGETTO=? AND U.FONTE_RIF=1 and not exists (select 1 from SIT_SOGGETTO_UNICO UU WHERE UU.ID_SOGGETTO=?)
/*
 * NON USATE
 */
...
SQL_INSERISCI_VIA_RIFERIMENTO=INSERT INTO SIT_VIA_UNICO (ID_VIA, INDIRIZZO, SEDIME, DT_INS, VALIDATO, CODICE_VIA) \
								...
								sysdate DT_INS, CODICE_VIA,1 \
								from SIT_VIA_TOTALE t where FK_ENTE_SORGENTE=? and prog_es=? \
								AND RATING is null AND nvl(FK_VIA,0) = '0' AND REL_DESCR is null AND ANOMALIA is null order by length(indirizzo) desc, sedime)							
...
SQL_LEGGI_VIE_UNICO_FUZZY=select * from (SELECT id_via idvia, UPPER (indirizzo) indirizzo, UPPER (sedime) sedime \
								  FROM sit_via_unico \
								 WHERE contains (indirizzo, 'fuzzy({@1}, 70, 6, weight)', 1) > 0 \
								 UNION \
								 SELECT id_via idvia, UPPER (indirizzo) indirizzo, UPPER (sedime) sedime \
								  FROM sit_via_unico \
								 WHERE contains (indirizzo, 'fuzzy({@2}, 70, 6, weight)', 1) > 0 \
								 UNION \
								 SELECT id_via idvia, UPPER (indirizzo) indirizzo, UPPER (sedime) sedime \
								  FROM sit_via_unico \
								 WHERE contains (indirizzo, 'fuzzy({@3}, 70, 6, weight)', 1) > 0 \
								 UNION \
								 SELECT id_via idvia, UPPER (indirizzo) indirizzo, UPPER (sedime) sedime \
								  FROM sit_via_unico \
								 WHERE indirizzo like '%@4%' \
								 )order by length(indirizzo) desc
...
SQL_CERCA_INDIRIZZO_UNICO_CON_PREFISSO=SELECT id_via idvia, UPPER (indirizzo) indirizzo, UPPER (sedime) sedime from sit_via_unico where sedime = ? and  indirizzo = ?
SQL_CERCA_INDIRIZZO_UNICO=SELECT id_via idvia, UPPER (indirizzo) indirizzo, UPPER (sedime) sedime from sit_via_unico where indirizzo = ?
...
SQL_CREA_VIA_UNICO_TEXT_INDEX=CREATE INDEX IDX_VIA_UNI_TEXT ON SIT_VIA_UNICO (INDIRIZZO) INDEXTYPE IS CTXSYS.CONTEXT
SQL_DROP_VIA_UNICO_TEXT_INDEX=DROP INDEX IDX_VIA_UNI_TEXT
SQL_SYNC_VIA_UNICO_TEXT_INDEX=begin ctx_ddl.SYNC_INDEX('IDX_VIA_UNI_TEXT'); end;
...
SQL_AGGIORNA_FK_VIA_CIVICI=UPDATE SIT_CIVICO_TOTALE CT SET FK_VIA = (SELECT v.fk_via FROM sit_via_totale v WHERE ct.ID_ORIG_VIA_TOTALE = v.ID_DWH \
							AND ct.fk_ente_sorgente = v.FK_ENTE_SORGENTE AND ct.PROG_ES = v.PROG_ES and v.indirizzo is not null and rownum <= 1) WHERE fk_via IS null
SQL_AGGIORNA_CIVICI_MODIFICATI=UPDATE SIT_CIVICO_TOTALE CT SET FK_CIVICO = NULL , REL_DESCR = NULL , RATING = NULL, NOTE = NULL, ATTENDIBILITA = NULL, \
								...
								and ct.fk_ente_sorgente = v.FK_ENTE_SORGENTE and ct.PROG_ES = v.PROG_ES and v.indirizzo is not null and rownum <= 1 and rownum <= 1) \
								where fk_via is not null and fk_via <> (select v.fk_via	from sit_via_totale v where ct.ID_ORIG_VIA_TOTALE = v.ID_DWH \
								and ct.fk_ente_sorgente = v.FK_ENTE_SORGENTE and ct.PROG_ES = v.PROG_ES and v.indirizzo is not null and rownum <=1) 							
