...
public class ViaCosapTassa extends DatoDwh implements Via{

	private Properties props = null;
	protected static final Logger log = Logger.getLogger(ViaCosapTassa.class.getName());
	
	
	public ViaCosapTassa(){
	...
	public String getSql(String processID) {
		...
		String sqlVia = this.getProperty("SQL_VIA_COSAP_TASSA");
		...
		try{			
			...
			if(iter.hasNext()){
				...
				if(isInvalid(sedime)){
					...
					sedime = civSedime1!=null ? civSedime1 : sedime;
				}
				note = civ.getNote();
			}
			...
			log.warn("Errore:Save Via Cosap Tassa="+e.getMessage(), e);
			Exception ea = new Exception("Errore:Save Via Cosap Tassa:"+e.getMessage());
			throw ea;
		}
	...
	public void prepareUpdateDato(DatoDwh classeFonte, Connection connForInsert, String insViaTotale, String updateViaTotale, String searchViaTotale, ResultSet rs, String codEnte, HashParametriConfBean paramConfBean) throws Exception {
		...
		try{			
			...
			log.warn("Errore:Update Via Cosap Tassa="+e.getMessage(), e);
			Exception ea = new Exception("Errore:Update Via Cosap Tassa:"+e.getMessage());
			throw ea;
		}
