package it.webred.ct.proc.ario.fonti.soggetto.versamenti.f24;
...
public class SoggettoF24VersContrib extends DatoDwh implements Soggetto  {

	
	private Properties props = null;
	private Properties propsCodSogg = null;
	
	protected static final Logger log = Logger.getLogger(SoggettoF24VersContrib.class.getName());
	
	
	public SoggettoF24VersContrib(){
	...
	public String getSql(String processID) {
		...
		String sqlSoggettoVersContrib = this.getProperty("SQL_SOGG_F24_VERS_CONTRIB");
	...
	public String getTable() {
		...
		String tabella = "SIT_T_F24_VERSAMENTI";		
		return tabella;
	}
	...
	public void prepareSaveDato(DatoDwh classeFonte, Connection connForInsert, String insSoggettoTotale, ResultSet rs, String codEnte, HashParametriConfBean paramConfBean) throws Exception {
		...
		try{			
			...
			String sData = rs.getString("DT_NASCITA");
			try{
				
				java.sql.Date data = new java.sql.Date(SDF.parse(sData).getTime());
			//	System.out.println("Data Convertita "+new SimpleDateFormat("dd/MM/yyyy").format(data));
				sst.setDtNascita(data);
			...
			log.warn("Errore:Save Soggetto Contribuente Versamenti F24="+e.getMessage(), e);
			Exception ea = new Exception("Errore:Save Soggetto Contribuente Versamenti F24:"+e.getMessage());
			throw ea;
		}
