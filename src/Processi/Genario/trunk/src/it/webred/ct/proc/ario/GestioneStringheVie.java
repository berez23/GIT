...
import it.webred.rulengine.db.RulesConnectionBase;
...
public class GestioneStringheVie {
	...
	public static Connection RE_CONN = null;
	...
	static
	{
		try
		{
			...
			try {			
			    ...
			    log.error("Eccezione romanToLetters.properties: "+e.getMessage(), e);			   
			}		
			...
			try {			
			    ...
			    log.error("Eccezione sedimario.properties: "+e.getMessage(), e);			   
			}		
			...
			// prendo alcuni sedime dalle tabelle sul db
			
			try{
				//Se ho lanciato genario da console, devo recuperare i parametri di connessione a RE dal file properties
				caricaProps();
				Connection conn = DriverManager.getConnection(props.getProperty("connRe.url"), props.getProperty("connRe.user"), props.getProperty("connRe.pass"));
				new RulesConnectionBase(conn);
			}catch(Exception e){
				System.out.println("Errore nella creazione della connessione a RE, secondo i parametri specificati nel main.properties:utilizzo la connessione del DS");
				log.debug("Errore nella creazione della connessione a RE, secondo i parametri specificati nel main.properties:utilizzo la connessione del DS");
			}
			
			//In ogni caso recupero le connessioni (se il processo è lanciato in JBoss recupera automaticamente il servizio RE altrimenti utilizza la connessione specificata nel main.properties
			...
			for (Iterator iterator = allC.iterator(); iterator.hasNext();) {
				...
				/*
				 * Codice inserito per far fronte all'errore di genario verificatosi 
				 * al verbano e causato dal loop qui sopra che tenta di fare la 
				 * stessa query da ogni connessione DWH_ disponibile in R_CONNECTION
				 * per recuperare le varie tipologie di sedime
				 */
				List<Object[]> results = null;
				try{
					results = (List<Object[]>) run.query(conn, sqlPrefissoVia, h);					
				}catch(Exception e2){
					log.warn("Errore nella lettura prefissi via (NON BLOCCANTE) Schema: " + conn.getMetaData().getUserName() + " [" + e2.getMessage() + "] " );
					continue;
				}
	...
	public static SitViaTotale removeCiviciOnIndirizzo(SitViaTotale svt) {
		...
		//problema es. Oristano via XXV LUGLIO 1943 dove il numero fa effettivamente parte del nome della via
		//se il nome della via finisce con un numero compreso tra 1000 e 2100 (presumibilmente anno) non viene trattato
		boolean anno = false;
		if (indirizzo != null && indirizzo.length() > 5) {
			String ctrl = indirizzo.substring(indirizzo.length() - 5, indirizzo.length()).trim();
			try {
				int ctrlInt = Integer.parseInt(ctrl);
				anno = ctrlInt >= 1000 && ctrlInt <= 2100;
			} catch (Exception e) {}
		}
		if (anno) {
			return svt;
		...
		if(indirizzo != null && !indirizzo.equals("")){
			...
			while(Character.isDigit(ch) || String.valueOf(ch).equals(" ")){				
				...
				try {
					...
					log.debug("removeCiviciOnIndirizzo() - impossibile rimuovere il civico dall'indirizzo: " + svt.getIndirizzo());
					return svt;
				}				
			}
		}
