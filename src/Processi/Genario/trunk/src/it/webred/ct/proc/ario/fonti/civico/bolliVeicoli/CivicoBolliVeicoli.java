package it.webred.ct.proc.ario.fonti.civico.bolliVeicoli;
...
public class CivicoBolliVeicoli extends DatoDwh implements Civici{

	private Properties props = null;
	protected static final Logger log = Logger.getLogger(CivicoBolliVeicoli.class.getName());
	
	
	public CivicoBolliVeicoli(){
	...
	public String getSql(String processID) {
		String sqlCivicoLocazioniI = this.getProperty("SQL_CIV_BOLLI_VEICOLI");
	...
	public void prepareSaveDato(DatoDwh classeFonte, Connection connForInsert, String insCivicoTotale, ResultSet rs, String codEnte, HashParametriConfBean paramConfBean) throws Exception {
		...
		try{									
			...
			log.warn("Errore:Save Civico Bolli Veicoli="+e.getMessage(), e);
			Exception ea = new Exception("Errore:Save Civico Bolli Veicoli:"+e.getMessage());
			throw ea;
		}
...
//				sct.setDataFineVal(dataFineVal);
			...
			log.warn("Errore:Update Civico Bolli Veicoli: "+e.getMessage(), e);
			Exception ea = new Exception("Errore:Update Civico Bolli Veicoli: "+e.getMessage());
			throw ea;
		}
