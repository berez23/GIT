...
public class AggregatoreFabbricati extends TipoAggregatore
{
	...
	PreparedStatement prsRiesegui = null;
	ResultSet rsRiesegui = null;
	...
	public void aggrega(String codEnte, HashParametriConfBean paramConfBean) throws AggregatoreException
	{
		...
		{
			log.error("Errore:AggregatoreFABBRICATI "+e.getMessage(), e);
	...
	protected void aggregazioneFonteRiferimento(Object[] fonte, String codEnte, HashParametriConfBean paramConfBean) throws Exception{
		...
		try{
			...
			throw new Exception("Errore in Aggregazione Fabbricati Riferimento: ", e);
		}
	}
	...
	protected void caricaDatiDaAggregareFonteRif() throws Exception{
		...
		try {
			...
			prsRiesegui = connectionForLongResultset.prepareStatement(sqlFabbricatoTotale);
	 ...
	 */
	private boolean  elabora(ResultSet resFabb, int critMaxFonte, String codEnte, HashParametriConfBean paramConfBean) throws Exception
	{
		...
		try {
			...
			while(resFabb.next() && !rieseguiQuery){
	...
	private void inserisciNonAgganciatiRif(Object[] fonte) throws AggregatoreException {
		...
		try {
			...
			while (rs.next()) {			
				...
				log.error("Errore:AggregatoreFabbricati inserisciNonAgganciati "+e.getMessage() ,e);
				AggregatoreException ea = new AggregatoreException(e.getMessage());
				throw ea;
		}
		finally {
	...
	private Object foglioPartRicerca(SitFabbricatoTotale sft, Object fk, int critMaxFonte) throws Exception {
		...
		if (isZero((String)p[1])){
			log.warn("Impossibile valutare se particella Zero " + p[0]+" "+p[1]);
			return fk;
		} 
			...
			if (fkRs==null){
				...
				for (SitFabbricatoTotale c : lFabbTotale){										
					...
					if(c.getFkFabbricato()!=null){
						...
						//il rating per i fabbricati è sempre 100, non c'è bisogno di valutarlo maggiore
	 ...
	 */
	private boolean elaboraAltreFonti(ResultSet resFabb, String codEnte, HashParametriConfBean paramConfBean) throws Exception
	{
