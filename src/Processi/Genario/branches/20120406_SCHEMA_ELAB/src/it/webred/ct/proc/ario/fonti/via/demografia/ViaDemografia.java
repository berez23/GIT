package it.webred.ct.proc.ario.fonti.via.demografia;
...
public class ViaDemografia extends DatoDwh implements Via{

	private Properties props = null;
	
	protected static final Logger log = Logger.getLogger(ViaDemografia.class.getName());
	
	
	public ViaDemografia(){
	...
	public String getSql(String processID) {
		...
		String sqlViaDemografia = this.getProperty("SQL_VIA_DEMOGRAFIA");
				
		if (processID != null && !processID.equals("")){
			sqlViaDemografia = sqlViaDemografia + " WHERE PROCESSID =?";			
		}
			
		return sqlViaDemografia;
	...
	public void prepareSaveDato(DatoDwh classeFonte, Connection connForInsert, String insViaTotale, ResultSet rs, String codEnte, HashParametriConfBean paramConfBean) throws Exception {
		...
		try{			
			...
			if (sedime1 != null)
				sedime = sedime1;
			String indirizzo = (rs.getString("DESCRIZIONE")!=null?rs.getString("DESCRIZIONE"):"-");
			...
			log.warn("Errore:Save Via Demografia="+e.getMessage());
			Exception ea = new Exception("Errore:Save Via Demografia:"+e.getMessage());
			throw ea;
		}
	...
	public void prepareUpdateDato(DatoDwh classeFonte, Connection connForInsert, String insViaTotale, String updateViaTotale, String searchViaTotale, ResultSet rs, String codEnte, HashParametriConfBean paramConfBean) throws Exception {
		...
		try{			
			...
			log.warn("Errore:Update Via Demografia="+e.getMessage());
			Exception ea = new Exception("Errore:Update Via Demografia:"+e.getMessage());
			throw ea;
		}
