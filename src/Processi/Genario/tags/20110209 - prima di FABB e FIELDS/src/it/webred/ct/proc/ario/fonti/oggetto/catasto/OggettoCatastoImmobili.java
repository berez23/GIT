	...
	public void prepareSaveDato(DatoDwh classeFonte, Connection connForInsert, String insOggettoTotale, ResultSet rs, String codEnte, HashParametriConfBean paramConfBean) throws Exception {
		...
		/*
		 ...
		 try{			
			...
			it.webred.utils.db.BasicRsPrcessor rsProcessor = new BasicRsPrcessor(SitOggettoTotale.class);
			sot = (SitOggettoTotale) PojoParser.rsToBean(rs, rsProcessor);			
			sot.getId().setProgEs(this.getProgEs());
			
			
			//caricaOggettoTotaleSpec(sot);
			
			//sot.setIdStorico(rs.getString("ID_STORICO"));
			//sot.setDataFineVal(rs.getDate("DT_FINE_VAL"));
			
			//Normalizzazione Dati
			nt.normalizzaOggetto(sot);
			
			//Calcolo Hash di chave
			//sot = setCtrHashSitOggettoTotale(sot);
			
			//Salvataggio
			super.saveSitOggettoTotale(classeFonte, connForInsert, insOggettoTotale, sot);

		}catch (Exception e) {
			log.warn("Errore:Save Oggetto Catasto1="+e.getMessage());
			Exception ea = new Exception("Errore:Save Oggetto Catasto1:"+e.getMessage());
			throw ea;
		}*/
