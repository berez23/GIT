...
import it.webred.rulengine.brick.loadDwh.load.rette.cancellazioni.v1.Env;
...
public class ImportBolletteCanc<T extends  Env<?>> extends ConcreteImport<T> {

	@Override
	public ConcreteImportEnv getEnvSpec(EnvImport ei) {
		return new Env<BolletteCancEnv>((BolletteCancEnv) ei);
	}
	...
	public boolean normalizza(String belfiore) throws RulEngineException {
		...
		try {
			...
			Command cmdSitRttCancBollette = CommandFactory.getCommand(it.webred.rulengine.brick.loadDwh.base.LoadSitRttCancBollette.class.getName(),true);

			String sql = concreteImportEnv.getSQL_RE_RTT_CANC_BOLLETTE_1_0();
			...
			while (rs.next()) {
				...
				// INSERIMENTO SIT_RTT_CANC_BOLLETTE
				EnvInsertDwh ec = concreteImportEnv.getEnvSitRttCancBollette();
				ec.setParametriPerGetRighe(rs.getTimestamp("DT_EXP_DATO"));
				InsertDwh.launchInserimento(con,ec, launch, cmdSitRttCancBollette,concreteImportEnv.getEnvImport().getConnectionName(),ctx, rs, abnormals, amkvext);
			}
		} catch (Exception e) {
			throw new RulEngineException("Errore durante normalizzazione CANC BOLLETTE", e);
		} 	finally {
			try {
				DbUtils.close(rs);
				DbUtils.close(ps);
