...
public class BolletteRateFiles<T extends BolletteRateEnv> extends ImportFilesXML<T> {

	private java.util.Date dataExport = null;

	public BolletteRateFiles(T env) {
		super(env);
	...
	public void preProcesing(Connection con) throws RulEngineException {
		...
		try {
			st = con.createStatement();
			st.execute(env.RE_RTT_RATE_BOLLETTE_IDX);
		} catch (SQLException e1) {
	...
	public String getNodeName() throws RulEngineException {
		return "/PagListaBollettaRata/PagBollettaRata";
	}
	...
	public void postElaborazione(String file, List<String> fileDaElaborare, String cartellaFiles) {
		...
		for(String altroFile: fileDaElaborare){
			if(altroFile.contains(data) && !altroFile.contains("TERMINATO_") && !altroFile.contains("BRA"))
