...
import it.webred.rulengine.brick.loadDwh.load.pubblicita.*;
...
public class ImportPubblicita <T extends Env<?>> extends ConcreteImport<T> {

	@Override
	public ConcreteImportEnv getEnvSpec(EnvImport ei) {
		return new Env<PubblicitaTipoRecordEnv<TestataStandardFileBean>>((PubblicitaTipoRecordEnv) ei);
	}
	...
	public boolean normalizza(String belfiore) throws RulEngineException {
		...
		try {
			...
			//SIT_PUB_PRAT_TESTATA
			Command cmdSitPubblicitaPratTestata = CommandFactory.getCommand(it.webred.rulengine.brick.loadDwh.base.LoadSitPubblicitaPratTestata.class.getName(), true);
			
			String sql = concreteImportEnv.getSQL_RE_PUBBLICITA_A();
			...
			while (rs.next()) {
				// INSERIMENTO SIT_PUB_PRAT_TESTATA
				EnvInsertDwh ec = concreteImportEnv.getEnvSitPubblicitaPratTestata();
				ec.setParametriPerGetRighe(rs.getTimestamp("DT_EXP_DATO"));
				InsertDwh.launchInserimento(con, ec, launch, cmdSitPubblicitaPratTestata, concreteImportEnv.getEnvImport().getConnectionName(), ctx, rs, abnormals, amkvext);
			}
