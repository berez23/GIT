...
public class LoadSitTIciSogg extends AbstractLoaderCommand implements Rule
{
	public LoadSitTIciSogg(BeanCommand bc, Properties jrulecfg) {
		super(bc, jrulecfg);
		// TODO Auto-generated constructor stub
	}


	private static final Logger log = Logger.getLogger(LoadSitTIciSogg.class.getName());
	
	//	blocco di inizializzazione della variabile tabellaDwhClass (specifica di quale classe è la TabellaDwh di questo load)
	{
		tabellaDwhClass = SitTIciSogg.class;
	}
	...
	throws CommandException
	{
		try {
			...
			if(id_orig==null || fk_ente_sorgente==null || dt_exp_dato==null || flag_dt_val_dato==null){
				return new RejectAck("SIT_T_ICI_SOGG - Dati obbligatori non forniti");
			}
			
			if(flag_dt_val_dato.intValue() != 0 && flag_dt_val_dato.intValue() != 1)
				return new RejectAck("SIT_T_ICI_SOGG - flag_dt_val_dato non valido");
			
			SitTIciSogg tab =  new SitTIciSogg();  
			...
			SitTIciSoggDao dao = (SitTIciSoggDao) DaoFactory.createDao(conn, tab, ctx.getEnteSorgenteById(fk_ente_sorgente)); 
		...
		{
			log.error("LoadSitTIciSogg",e);
			ErrorAck ea = new ErrorAck(e.getMessage());
			return (ea);
		}
	
		return(new ApplicationAck("Record SIT_T_ICI_SOGG inserito"));
