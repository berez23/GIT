...
import it.webred.rulengine.brick.loadDwh.load.rette.rate.v1.Env;
...
public class ImportBolletteRate<T extends  Env<?>> extends ConcreteImport<T> {

	@Override
	public ConcreteImportEnv getEnvSpec(EnvImport ei) {
		return new Env<BolletteRateEnv>((BolletteRateEnv) ei);
	}
	...
	public boolean normalizza(String belfiore) throws RulEngineException {
		...
		try {
			...
			Command cmdSitRttRateBollette = CommandFactory.getCommand(it.webred.rulengine.brick.loadDwh.base.LoadSitRttRateBollette.class.getName(),true);

			String sql = concreteImportEnv.getSQL_RE_RTT_RATE_BOLLETTE_1_0();
			...
			while (rs.next()) {
				...
				// INSERIMENTO SIT_RTT_RATE_BOLLETTE
				EnvInsertDwh ec = concreteImportEnv.getEnvSitRttRateBollette();
				ec.setParametriPerGetRighe(rs.getTimestamp("DT_EXP_DATO"));
				InsertDwh.launchInserimento(con,ec, launch, cmdSitRttRateBollette,concreteImportEnv.getEnvImport().getConnectionName(),ctx, rs, abnormals, amkvext);
			}
		} catch (Exception e) {
			throw new RulEngineException("Errore durante normalizzazione RATE BOLLETTE", e);
		} 	finally {
			try {
				DbUtils.close(rs);
				DbUtils.close(ps);
