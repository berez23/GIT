...
public class EnvSitCConcIndirizzi
		extends
		it.webred.rulengine.brick.loadDwh.load.concessioni.v1.EnvSitCConcIndirizzi {

	public EnvSitCConcIndirizzi(String nomeTabellaOrigine, String provenienza,
	...
	public ArrayList<RigaToInsert> getRigheSpec(ResultSet rs) throws Exception {
		...
		String chiave_a = null;
		try {
			chiave_a = rs.getString("CHIAVE_A");
		} catch (Exception e) {
			chiave_a = rs.getString("CHIAVE");
		}
		params.put("ID_ORIG_C_CONCESSIONI", chiave_a);
	...
	public String getSqlUpdateFlagElaborato() {
		return "UPDATE "
				...
				+ " SET RE_FLAG_ELABORATO='1' WHERE CHIAVE=? AND DT_EXP_DATO=? AND RE_FLAG_ELABORATO='0'";
	...
	public void executeSqlPostInsertRecord(Connection conn,
		...
		/*
		 * // // praticamente storicizzo eventuali records già presente per la
		 * stessa concessione // in qesta tabella // PreparedStatement pse =
		 * null; try { String sql = "         UPDATE SIT_C_CONC_INDIRIZZI i  "
		 * +" set dt_fine_val =? " +" where dt_fine_val is null "
		 * +" and exists  "
		 ...
		 * +"    and c.id_orig = ? " +"    and ii.dt_inizio_val  = ?   "
		 * +"    and i.dt_inizio_val < ii.dt_inizio_val " +"  )"; try { pse =
		 * conn.prepareStatement(sql); pse.setTimestamp(1,
		 * (Timestamp)currRecord.get("DT_EXP_DATO")); pse.setString(2,
		 * (String)currRecord.get("ID_ORIG_C_CONCESSIONI")); pse.setTimestamp(3,
		 * (Timestamp)currRecord.get("DT_EXP_DATO"));
		 * 
		 * pse.execute();
		 * 
		 * } finally { if(pse!=null) pse.close(); } } finally {
		 * 
		 * }
		 */
