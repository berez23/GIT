...
import it.webred.rulengine.brick.loadDwh.load.scuole.istituti.v1.Env;
...
public class ImportSclIstituti<T extends  Env<?>> extends ConcreteImport<T> {

	@Override
	public ConcreteImportEnv getEnvSpec(EnvImport ei) {
		return new Env<SclIstitutiEnv>((SclIstitutiEnv) ei);
	}
	...
	public boolean normalizza(String belfiore) throws RulEngineException {
		...
		try {
			...
			Command cmdSitSclIstituti = CommandFactory.getCommand(it.webred.rulengine.brick.loadDwh.base.LoadSitSclIstituti.class.getName(),true);

			String sql = concreteImportEnv.getSQL_RE_SCL_ISTITUTI_1_0();
			...
			while (rs.next()) {
				...
				// INSERIMENTO SIT_SCL_ISTITUTI
				EnvInsertDwh ec = concreteImportEnv.getEnvSitSclIstituti();
				ec.setParametriPerGetRighe(rs.getTimestamp("DT_EXP_DATO"));
				InsertDwh.launchInserimento(con,ec, launch, cmdSitSclIstituti,concreteImportEnv.getEnvImport().getConnectionName(),ctx, rs, abnormals, amkvext);
			}
		} catch (Exception e) {
			throw new RulEngineException("Errore durante normalizzazione SCL_ISTITUTI", e);
		} 	finally {
			try {
				DbUtils.close(rs);
				DbUtils.close(ps);
