...
public class BolletteCancFiles<T extends BolletteCancEnv> extends ImportFilesXML<T> {

	private java.util.Date dataExport = null;
	
	public BolletteCancFiles(T env) {
		super(env);
	...
	public void preProcesing(Connection con) throws RulEngineException {
		...
		try {
			st = con.createStatement();
			st.execute(env.RE_RTT_CANC_BOLLETTE_IDX);
		} catch (SQLException e1) {
	...
	/*
		ListaCancBolletta.xsd" versione="1.0">
			<CancBolletta progressivo="0">
				<IdServizio>103001</IdServizio>
				<CodBolletta>0910EL0000012129</CodBolletta>
				<CodBolletta>0910EL0000012172</CodBolletta>
				<CodBolletta>0910EL0000012194</CodBolletta>
			</CancBolletta>
			<CancBolletta progressivo="1">
				<IdServizio>103001</IdServizio>
				<CodBolletta>a1</CodBolletta>
				<CodBolletta>a2</CodBolletta>
				<CodBolletta>a3</CodBolletta>
			</CancBolletta>
		</ListaCancBolletta>
		*/
	@Override
	public String getNodeName() throws RulEngineException {
		return "/ListaCancBolletta/CancBolletta";
	}
	...
	public void postElaborazione(String file, List<String> fileDaElaborare, String cartellaFiles) {
		...
		for(String altroFile: fileDaElaborare){
			if(altroFile.contains(data) && !altroFile.contains("TERMINATO_") && !altroFile.contains("CNB"))
