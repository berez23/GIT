...
import it.webred.rulengine.dwh.Dao.SitRuoloTarsuStDao;
...
public class LoadSitRuoloTarsuSt extends AbstractLoaderCommand implements Rule
{
	
	
	public LoadSitRuoloTarsuSt(BeanCommand bc, Properties jrulecfg) {
		super(bc, jrulecfg);
	}


	private static final Logger log = Logger.getLogger(LoadSitRuoloTarsuSt.class.getName());
	
	//	blocco di inizializzazione della variabile tabellaDwhClass (specifica di quale classe è la TabellaDwh di questo load)
	{
		tabellaDwhClass = SitRuoloTarsuSt.class;
	}
	...
	throws CommandException
	{
		...
		try {
			...
			SitRuoloTarsuSt tab = (SitRuoloTarsuSt)getTabellaDwhInstance(ctx);
			...
			SitRuoloTarsuStDao dao = (SitRuoloTarsuStDao) DaoFactory.createDao(conn, tab, ctx.getEnteSorgenteById(fk_ente_sorgente)); 
		...
		{
			log.error("LoadSitRuoloTarsuSt",e);
			ErrorAck ea = new ErrorAck(e.getMessage());
			return (ea);
		}
	
		return(new ApplicationAck("Record SIT_RUOLO_TARSU_ST inserito"));
