...
import it.webred.rulengine.brick.loadDwh.load.ruolo.tares.RTaresFiles;
...
public class ImportRTaresRule extends Command implements Rule {
	
	private static final org.apache.log4j.Logger log = Logger.getLogger(ImportRTaresRule.class.getName());

	public ImportRTaresRule(BeanCommand bc, Properties jrulecfg) {
		super(bc, jrulecfg);
	...
	public CommandAck run(Context ctx) throws CommandException {
		...
		try {		
			RTaresEnv env = new RTaresEnv((String)ctx.get("connessione") , ctx);
			RTaresFiles<RTaresEnv> r = new RTaresFiles<RTaresEnv>(env);
