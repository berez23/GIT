...
public class BolletteDettFiles<T extends BolletteDettEnv> extends ImportFilesXML<T> {

	private java.util.Date dataExport = null;
	
	public BolletteDettFiles(T env) {
		super(env);
	...
	public void preProcesing(Connection con) throws RulEngineException {
		...
		try {
			st = con.createStatement();
			st.execute(env.RE_RTT_DETT_BOLLETTE_IDX);
		} catch (SQLException e1) {
	...
	public String getNodeName() throws RulEngineException {
		return "/PagListaBollettaDett/PagBollettaDett";
	}

	//metodi per il parsing di file xml tipo:
	/*<PagBollettaDett progressivo="1">
	<CodBolletta>0909ME0000003489</CodBolletta>
	<DtIniServizio>2009-10-27</DtIniServizio>
	<DtFinServizio>2009-11-20</DtFinServizio>
	<ListaDettaglio>
		<Dettaglio progressivo="1">
			<DesVoce>Descrizione</DesVoce>
			<Valore>Servizio Ristorazione Scolastica - Costo pasti mese di Settembre 2009 - alunno STANKOVIC DENIS</Valore>
		</Dettaglio>
		<Dettaglio progressivo="2">
			<DesVoce></DesVoce>
			<Valore>Giorni consumo pasto: 23 30</Valore>
		</Dettaglio>
		...
	</ListaDettaglio>
	</PagBollettaDett>*/
	...
	public List<String> getNodeToExclude() throws RulEngineException {
		List<String> nodeToExclude = new ArrayList<String>();
		
		nodeToExclude.add("ListaDettaglio");
	...
	public void postElaborazione(String file, List<String> fileDaElaborare, String cartellaFiles) {
		...
		for(String altroFile: fileDaElaborare){
			if(altroFile.contains(data) && !altroFile.contains("TERMINATO_") && !altroFile.contains("BDE"))
