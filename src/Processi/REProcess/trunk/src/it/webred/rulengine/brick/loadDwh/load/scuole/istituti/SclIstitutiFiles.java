...
public class SclIstitutiFiles<T extends SclIstitutiEnv> extends ImportFilesFlat<T> {

	public SclIstitutiFiles(T env) {
		super(env);
	...
	public void preProcesing(Connection con) throws RulEngineException {
		...
		try {
			st = con.createStatement();
			st.execute(env.RE_SCL_ISTITUTI_IDX);
		} catch (SQLException e1) {
	...
	public boolean filtroFile(String file, List<String> fileDaElaborare, String cartellaDati) {
		...
		if (file.startsWith("scuo") && file.endsWith("txt")) {
