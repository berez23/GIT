...
public class SclClassiFiles<T extends SclClassiEnv> extends ImportFilesFlat<T> {

	public SclClassiFiles(T env) {
		super(env);
	...
	public void preProcesing(Connection con) throws RulEngineException {
		...
		try {
			st = con.createStatement();
			st.execute(env.RE_SCL_CLASSI_IDX);
		} catch (SQLException e1) {
	...
	public boolean filtroFile(String file, List<String> fileDaElaborare, String cartellaDati) {
		...
		if (file.startsWith("clas") && file.endsWith("txt")) {
