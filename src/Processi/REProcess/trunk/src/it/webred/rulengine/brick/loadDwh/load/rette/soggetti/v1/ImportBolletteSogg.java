...
import it.webred.rulengine.brick.loadDwh.load.rette.soggetti.v1.Env;
...
public class ImportBolletteSogg<T extends  Env<?>> extends ConcreteImport<T> {

	@Override
	public ConcreteImportEnv getEnvSpec(EnvImport ei) {
		return new Env<BolletteSoggEnv>((BolletteSoggEnv) ei);
	}
	...
	public boolean normalizza(String belfiore) throws RulEngineException {
		...
		try {
			...
			Command cmdSitRttSoggBollette = CommandFactory.getCommand(it.webred.rulengine.brick.loadDwh.base.LoadSitRttSoggBollette.class.getName(),true);

			String sql = concreteImportEnv.getSQL_RE_RTT_SOGG_BOLLETTE_1_0();
			...
			while (rs.next()) {
				...
				// INSERIMENTO SIT_RTT_SOGG_BOLLETTE
				EnvInsertDwh ec = concreteImportEnv.getEnvSitRttSoggBollette();
				ec.setParametriPerGetRighe(rs.getTimestamp("DT_EXP_DATO"));
				InsertDwh.launchInserimento(con,ec, launch, cmdSitRttSoggBollette,concreteImportEnv.getEnvImport().getConnectionName(),ctx, rs, abnormals, amkvext);
			}
		} catch (Exception e) {
			throw new RulEngineException("Errore durante normalizzazione SOGG BOLLETTE", e);
		} 	finally {
			try {
				DbUtils.close(rs);
				DbUtils.close(ps);
