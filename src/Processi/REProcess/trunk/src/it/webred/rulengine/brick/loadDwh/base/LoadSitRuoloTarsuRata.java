...
import it.webred.rulengine.dwh.Dao.SitRuoloTarsuRataDao;
...
public class LoadSitRuoloTarsuRata extends AbstractLoaderCommand implements Rule
{
	
	
	public LoadSitRuoloTarsuRata(BeanCommand bc, Properties jrulecfg) {
		super(bc, jrulecfg);
	}
	...
	{
		tabellaDwhClass = SitRuoloTarsuRata.class;
	}
	...
	{	
		...
		try {
			...
			if(fk_ente_sorgente==null || dt_exp_dato==null || flag_dt_val_dato==null)
				return new RejectAck("SIT_RUOLO_TARSU_RATA - Dati obbligatori non forniti");
			if(flag_dt_val_dato.intValue() != 0 && flag_dt_val_dato.intValue() != 1)
				return new RejectAck("SIT_RUOLO_TARSU_RATA - flag_dt_val_dato non valido");
			
			SitRuoloTarsuRata tab = (SitRuoloTarsuRata)getTabellaDwhInstance(ctx);
			...
			SitRuoloTarsuRataDao dao = (SitRuoloTarsuRataDao) DaoFactory.createDao(conn, tab, ctx.getEnteSorgenteById(fk_ente_sorgente)); 
		...
		{
			log.error("LoadSitRuoloTarsuRata",e);
			ErrorAck ea = new ErrorAck(e.getMessage());
			return (ea);
		}
	
		return(new ApplicationAck("Record SIT_RUOLO_TARSU_RATA inserito"));
