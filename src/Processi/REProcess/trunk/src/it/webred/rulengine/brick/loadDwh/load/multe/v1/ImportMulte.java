...
import it.webred.rulengine.brick.loadDwh.load.multe.v1.Env;
...
public class ImportMulte<T extends  Env<?>> extends ConcreteImport<T> {

	@Override
	public ConcreteImportEnv getEnvSpec(EnvImport ei) {
		return new Env<MulteEnv>((MulteEnv) ei);
	}
	...
	public boolean normalizza(String belfiore) throws RulEngineException {
		...
		try {
			...
			Command cmdSitTrffMulte = CommandFactory.getCommand(it.webred.rulengine.brick.loadDwh.base.LoadSitTrffMulte.class.getName(),true);

			String sql = concreteImportEnv.getSQL_RE_TRFF_MULTE_1_0();
			...
			while (rs.next()) {
				...
				// INSERIMENTO SIT_TRFF_MULTE
				EnvInsertDwh ec = concreteImportEnv.getEnvSitTrffMulte();
				ec.setParametriPerGetRighe(rs.getTimestamp("DT_EXP_DATO"));
				InsertDwh.launchInserimento(con,ec, launch, cmdSitTrffMulte,concreteImportEnv.getEnvImport().getConnectionName(),ctx, rs, abnormals, amkvext);
			}
		} catch (Exception e) {
			throw new RulEngineException("Errore durante normalizzazione MULTE", e);
		} 	finally {
			try {
				DbUtils.close(rs);
				DbUtils.close(ps);
