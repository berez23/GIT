...
import it.webred.rulengine.dwh.Dao.SitTTarRiduzioniDao;
...
public class LoadSitTTarRiduzioni extends AbstractLoaderCommand implements Rule
{
	public LoadSitTTarRiduzioni(BeanCommand bc, Properties jrulecfg) {
		super(bc, jrulecfg);
		// TODO Auto-generated constructor stub
	}


	private static final Logger log = Logger.getLogger(LoadSitTTarRiduzioni.class.getName());
	
	//	blocco di inizializzazione della variabile tabellaDwhClass (specifica di quale classe è la TabellaDwh di questo load)
	{
		tabellaDwhClass = SitTTarRiduzioni.class;
	}
	...
	throws CommandException
	{
		try {
			...
			if(/*id_orig==null || */fk_ente_sorgente==null || dt_exp_dato==null || flag_dt_val_dato==null)
				return new RejectAck("SIT_T_TAR_RIDUZIONI - Dati obbligatori non forniti");
			if(flag_dt_val_dato.intValue() != 0 && flag_dt_val_dato.intValue() != 1)
				return new RejectAck("SIT_T_TAR_RIDUZIONI - flag_dt_val_dato non valido");
			
			SitTTarRiduzioni tab = new SitTTarRiduzioni();			
			...
			SitTTarRiduzioniDao dao = (SitTTarRiduzioniDao) DaoFactory.createDao(conn, tab, ctx.getEnteSorgenteById(fk_ente_sorgente)); 
		...
		{
			log.error("LoadSitTTarRiduzioni",e);
			ErrorAck ea = new ErrorAck(e.getMessage());
			return (ea);
		}
	
		return(new ApplicationAck("Record SIT_T_TAR_RIDUZIONI inserito"));
