...
import it.webred.rulengine.brick.loadDwh.load.demografia.anagrafe.DemogAnagrafeFiles;
import it.webred.rulengine.db.model.RAbNormal;
import it.webred.rulengine.exception.CommandException;
import it.webred.rulengine.impl.bean.BeanCommand;
import it.webred.utils.GenericTuples.T2;

public class ImportDemogAnagrafeRule extends Command implements Rule {
	
	private static Logger log = Logger.getLogger(ImportDemogAnagrafeRule.class.getName());
	
	public ImportDemogAnagrafeRule(BeanCommand bc, Properties jrulecfg) {
		super(bc, jrulecfg);
	...
	public CommandAck run(Context ctx) throws CommandException {
		...
		try {
			DemogAnagrafeEnv env = new DemogAnagrafeEnv((String)ctx.get("connessione"), ctx);
			DemogAnagrafeFiles<DemogAnagrafeEnv> danag = new DemogAnagrafeFiles<DemogAnagrafeEnv>(env);
