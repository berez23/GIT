...
import it.webred.rulengine.dwh.Dao.SitRuoloTaresStSgDao;
...
public class LoadSitRuoloTaresStSg extends AbstractLoaderCommand implements Rule
{
	
	
	public LoadSitRuoloTaresStSg(BeanCommand bc, Properties jrulecfg) {
		super(bc, jrulecfg);
	}


	private static final Logger log = Logger.getLogger(LoadSitRuoloTaresStSg.class.getName());
	
	//	blocco di inizializzazione della variabile tabellaDwhClass (specifica di quale classe è la TabellaDwh di questo load)
	{
		tabellaDwhClass = SitRuoloTaresStSg.class;
	}
	...
	throws CommandException
	{
		...
		try {
			...
			if(fk_ente_sorgente==null || dt_exp_dato==null || flag_dt_val_dato==null)
				return new RejectAck("SIT_RUOLO_TARES_ST_SG - Dati obbligatori non forniti");
			if(flag_dt_val_dato.intValue() != 0 && flag_dt_val_dato.intValue() != 1)
				return new RejectAck("SIT_RUOLO_TARES_ST_SG - flag_dt_val_dato non valido");
			
			SitRuoloTaresStSg tab = (SitRuoloTaresStSg)getTabellaDwhInstance(ctx);
			...
			SitRuoloTaresStSgDao dao = (SitRuoloTaresStSgDao) DaoFactory.createDao(conn, tab, ctx.getEnteSorgenteById(fk_ente_sorgente)); 
		...
		{
			log.error("LoadSitRuoloTaresStSg",e);
			ErrorAck ea = new ErrorAck(e.getMessage());
			return (ea);
		}
	
		return(new ApplicationAck("Record SIT_RUOLO_TARES_ST_SG inserito"));
