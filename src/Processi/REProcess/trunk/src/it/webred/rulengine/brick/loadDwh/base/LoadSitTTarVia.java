...
import it.webred.rulengine.dwh.Dao.SitTTarViaDao;
...
public class LoadSitTTarVia extends AbstractLoaderCommand implements Rule
{
	public LoadSitTTarVia(BeanCommand bc, Properties jrulecfg) {
		super(bc, jrulecfg);
		// TODO Auto-generated constructor stub
	}


	private static final Logger log = Logger.getLogger(LoadSitTTarVia.class.getName());
	
	//	blocco di inizializzazione della variabile tabellaDwhClass (specifica di quale classe è la TabellaDwh di questo load)
	{
		tabellaDwhClass = SitTTarVia.class;
	}
	...
	throws CommandException
	{
		try {
			...
			if(id_orig==null || fk_ente_sorgente==null || dt_exp_dato==null || flag_dt_val_dato==null)
				return new RejectAck("SIT_T_TAR_VIA - Dati obbligatori non forniti");
			if(flag_dt_val_dato.intValue() != 0 && flag_dt_val_dato.intValue() != 1)
				return new RejectAck("SIT_T_TAR_VIA - flag_dt_val_dato non valido");
			
			SitTTarVia tab = new SitTTarVia();
			...
			SitTTarViaDao dao = (SitTTarViaDao) DaoFactory.createDao(conn, tab, ctx.getEnteSorgenteById(fk_ente_sorgente)); 
		...
		{
			log.error("LoadSitTTarVia",e);
			ErrorAck ea = new ErrorAck(e.getMessage());
			return (ea);
		}
	
		return(new ApplicationAck("Record SIT_T_TAR_VIA inserito"));
