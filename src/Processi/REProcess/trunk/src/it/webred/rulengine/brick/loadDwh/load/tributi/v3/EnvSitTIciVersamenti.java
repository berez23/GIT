...
public class EnvSitTIciVersamenti extends EnvInsertDwh {
	...
	public EnvSitTIciVersamenti(String nomeTabellaOrigine, String provenienza, String[] nomeCampoChiave) {
	...
	public ArrayList<RigaToInsert> getRigheSpec(ResultSet rs) throws Exception {
		...
		params.put("ID_ORIG", rs.getString("ID_ORIG_VICI"));
		params.put("FK_ENTE_SORGENTE", ID_ENTE_SORGENTE);		
		params.put("ID_ORIG_SOGG_ICI", rs.getString("ID_ORIG_SOGG_ICI"));
		...
		params.put("DAT_PAG", rs.getObject("DAT_PAG") == null ? null : new Timestamp(SDF.parse(rs.getString("DAT_PAG")).getTime()));
		params.put("IMP_PAG_EU", rs.getObject("IMP_PAG_EU") == null ? null : new BigDecimal("" + DF.parse(rs.getString("IMP_PAG_EU")).doubleValue()));		
		params.put("IMP_TER_AGR_EU", rs.getObject("IMP_TER_AGR_EU") == null ? null : new BigDecimal("" + DF.parse(rs.getString("IMP_TER_AGR_EU")).doubleValue()));
		params.put("IMP_ARE_FAB_EU", rs.getObject("IMP_ARE_FAB_EU") == null ? null : new BigDecimal("" + DF.parse(rs.getString("IMP_ARE_FAB_EU")).doubleValue()));
		params.put("IMP_ABI_PRI_EU", rs.getObject("IMP_ABI_PRI_EU") == null ? null : new BigDecimal("" + DF.parse(rs.getString("IMP_ABI_PRI_EU")).doubleValue()));
		params.put("IMP_ALT_FAB_EU", rs.getObject("IMP_ALT_FAB_EU") == null ? null : new BigDecimal("" + DF.parse(rs.getString("IMP_ALT_FAB_EU")).doubleValue()));
		params.put("IMP_DTR_EU", rs.getObject("IMP_DTR_EU") == null ? null : new BigDecimal("" + DF.parse(rs.getString("IMP_DTR_EU")).doubleValue()));		
		params.put("NUM_FAB", rs.getObject("NUM_FAB") == null ? null : new BigDecimal(DF.parse(rs.getString("NUM_FAB")).doubleValue()));
		params.put("FLG_ACC_SAL", rs.getString("FLG_ACC_SAL"));
		params.put("NUM_BOLL", rs.getObject("NUM_BOLL") == null ? null : new BigDecimal(DF.parse(rs.getString("NUM_BOLL")).doubleValue()));
		params.put("TIP_PAG", rs.getString("TIP_PAG"));
	...
	public String getSqlUpdateFlagElaborato() {
		return "UPDATE " + getNomeTabellaOrigine() + " SET RE_FLAG_ELABORATO='1' WHERE ID_ORIG_VICI=? AND DT_EXP_DATO=? AND RE_FLAG_ELABORATO='0'";
	}
