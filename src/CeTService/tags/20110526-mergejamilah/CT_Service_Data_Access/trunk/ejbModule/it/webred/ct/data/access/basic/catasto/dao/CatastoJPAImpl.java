...
import it.webred.ct.data.model.catasto.AnagSoggetti;
import it.webred.ct.data.model.catasto.ConsConsTab;
	...
	public List<SoggettoDTO> getListaSoggettiImmobile(RicercaOggettoCatDTO ro) {
		...
		try {
			...
			for (Object[] rsSogg : listSogg) {
				...
				if (tipo_sogg.equalsIgnoreCase("P")|| (cf != null && !cf.trim().equals("") && !cf.trim().equals("-") )) 
		...
		try {
			...
			for (Sititrkc sititrkc: listaTerreni){
				...
				if (codiQual != null){
					...
					q = manager_siti.createNamedQuery("SitiCodsQual");
					q.setParameter("codiQual", codiQual);
					
					String descQual= (String)q.getSingleResult();
					
					sititrkc.setDescQualita(descQual);
