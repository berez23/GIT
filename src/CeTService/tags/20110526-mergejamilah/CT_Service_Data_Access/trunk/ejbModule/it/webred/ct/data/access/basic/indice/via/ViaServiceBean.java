...
public class ViaServiceBean extends IndiceBaseService implements ViaService {
	...
	public void aggregaUnici(IndiceDataIn indDataIn) {
		...
		try {
			...
			/*q = manager.createNamedQuery("SitViaTotale.getViaTotaleByUnico");
			...
			// associazione civici unico a nuovo unico
			/*Query q2 = manager.createNamedQuery("SitCivicoUnico.getCivicoUnicoByViaUnico");
			q2.setParameter("idVia", idUno);
			List<SitCivicoUnico> result2 = q2.getResultList();*/
			
			List<SitCivicoUnico> result2 = viaDAO.getCivicoUnicoByViaUnico(idUno);
			
			
			/*q2 = manager.createNamedQuery("SitCivicoUnico.getCivicoUnicoByViaUnico");
			q2.setParameter("idVia", idDue);
			result.addAll(q2.getResultList());*/
			
			List<SitCivicoUnico> civUniByViaUni = viaDAO.getCivicoUnicoByViaUnico(idDue);
			result2.addAll(civUniByViaUni);
			

			for (SitCivicoUnico civico : result2) {
				civico.setFkVia(new BigDecimal(nuovo.getIdVia()));
				//manager.merge(civico);
				viaDAO.mergeCivicoUnico(civico);
			}

			// associazione civici totale a nuovo unico
			/*Query q3 = manager.createNamedQuery("SitCivicoTotale.getCivicoTotaleByViaUnico");
			q3.setParameter("idVia", idUno);
			List<SitCivicoTotale> result3 = q3.getResultList();*/
			
			List<SitCivicoTotale> result3 = viaDAO.getCivicoTotaleByViaUnico(idUno);
			
			
			/*q3 = manager.createNamedQuery("SitCivicoTotale.getCivicoTotaleByViaUnico");
			q3.setParameter("idVia", idDue);
			result.addAll(q3.getResultList());*/
			
			List<SitCivicoTotale> civTotByViaUnico = viaDAO.getCivicoTotaleByViaUnico(idDue);
			result3.addAll(civTotByViaUnico);
			

			for (SitCivicoTotale civico : result3) {
				civico.setFkVia(new BigDecimal(nuovo.getIdVia()));
				
				//manager.merge(civico);
				viaDAO.mergeCivico(civico);
			}
