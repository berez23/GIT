...
public class VerificaReperimentoHandler extends AbstractVerificaHandler {
		...
		try {
			if(prevs != null) {
				for(RProcessMonitor prev: prevs) {
					...
					if(prev.getRCommand().getRCommandType().getId() == super.REPERIMENTO) {
					...
					switch(statoFD.getId().intValue()) {
						...
						*/
						...
						default: {
							//reperimento può essere eseguito
							break;
						}
					}
		...
		try {
			...
			if(rpm != null) {
				...
				switch(rpm.getRAnagStato().getId().intValue()) {
					case 1 : {
						//reprimento non può essere eseguito
						vhi.setEsito(false);
