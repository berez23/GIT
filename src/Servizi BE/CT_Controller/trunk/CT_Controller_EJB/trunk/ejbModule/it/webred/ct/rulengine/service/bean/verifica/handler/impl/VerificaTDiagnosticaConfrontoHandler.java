...
public class VerificaTDiagnosticaConfrontoHandler extends AbstractVerificaHandler {
		...
		try {
			if(prevs != null) {
				for(RProcessMonitor prev: prevs) {
					...
					if(prev.getRCommand().getRCommandType().getId() == super.D_CONFRONTO) {
		...
		try {
			...
			if(rpm != null) {
				...
				switch(rpm.getRAnagStato().getId().intValue()) {
					case 8 : {
						//trattamento non può essere eseguito
						vhi.setEsito(false);
