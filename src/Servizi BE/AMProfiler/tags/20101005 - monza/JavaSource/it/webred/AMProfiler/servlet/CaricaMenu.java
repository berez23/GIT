 ...
 * 
 */
public class CaricaMenu extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
	/**
	 * 
	 */
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			boolean gestPerm = GestionePermessi.autorizzato(user, CodificaPermessi.NOME_APP, CodificaPermessi.ITEM_MAPPING, CodificaPermessi.GESTIONE, true);
			...
			try {
				...
				ArrayList listaApplUtente = GestionePermessi.getApplications(user, true);

				con = BaseAction.apriConnessione();
				statment = con.createStatement();
				ResultSet rsc = statment.executeQuery("select distinct belfiore, descrizione from am_comuni");
				Hashtable<String, String> decodeComuni = new Hashtable<String, String>();
				while (rsc.next()) {
					decodeComuni.put(rsc.getString("belfiore"), rsc.getString("descrizione"));
				}
				rsc.close();

				ResultSet rs = statment.executeQuery("select distinct APP_TYPE from am_application where APP_TYPE !='AMProfiler' order by APP_TYPE");
				...
				if (intestazioneTab.size() != 1) {
					...
					rs = statment.executeQuery("select * from AM_APPLICATION where NAME != 'AMProfiler' order by FK_AM_COMUNI, APP_CATEGORY, NAME");
					HashMap<String, Boolean> permessoDiAccessoAdComune = new HashMap<String, Boolean>();

					while (rs.next()) {
						String comune = rs.getString("FK_AM_COMUNI");
					...
					request.setAttribute("myMenu", getMyMenu(mappa, decodeComuni, gestPerm, gestServizi));
				}
	...
	private String getMyMenu(LinkedHashMap<String, LinkedHashMap> mappa, Hashtable<String, String> decodeComuni, boolean gestPermessi, boolean gestServizi) {
		...
		for (String key : mappa.keySet()) {
			...
			for (Object k : app.keySet()) {
				...
				for (AmApplication a : l) {
					if (a.getCat_app().equals("1")) {
						...
						if (gestPermessi) {
							appAcq.append("<a href=\"CaricaPermessi?application=" + a.getName() + "&appType=" + a.getTipo_app() + "\"><img style=\"border:none;\" src=\"img/keys.gif\" ></img></a>");
						}
						
						if (gestServizi) {
							appAcq.append("<a href=\"CaricaServizi?application=" + a.getName() + "&appType=" + a.getTipo_app() + "\"><img style=\"border:none;\" src=\"img/services.png\" ></img></a>");
						}
						...
						appVis.append("				<img border=\"0\" src=\"jsp/img/" + a.getTipo_app().toLowerCase() + ".gif\" width=\"150\" height=\"150\" />\n");
						if (href != null) {
							appVis.append("</a>");
						}
						appVis.append("				</div>\n");
						...
						if (gestPermessi) {
							appVis.append("<a href=\"CaricaPermessi?application=" + a.getName() + "&appType=" + a.getTipo_app() + "\"><img style=\"border:none;\" src=\"img/keys.gif\" ></img></a>");
						}

						if (gestServizi) {
							appVis.append("<a href=\"CaricaServizi?application=" + a.getName() + "&appType=" + a.getTipo_app() + "\"><img style=\"border:none;\" src=\"img/services.png\" ></img></a>");
						}
						...
						appVert.append("				<img border=\"0\" src=\"jsp/img/" + a.getTipo_app().toLowerCase() + ".gif\" width=\"150\" height=\"150\" />\n");
						if (href != null) {
							appVert.append("</a>");
						}
						appVert.append("				</div>\n");
						...
						if (gestPermessi) {
							appVert.append("<a href=\"CaricaPermessi?application=" + a.getName() + "&appType=" + a.getTipo_app() + "\"><img style=\"border:none;\" src=\"img/keys.gif\" ></img></a>");
						}

						if (gestServizi) {
							appVert.append("<a href=\"CaricaServizi?application=" + a.getName() + "&appType=" + a.getTipo_app() + "\"><img style=\"border:none;\" src=\"img/services.png\" ></img></a>");
						}
						...
						if (gestPermessi) {
							appAltri.append("<a href=\"CaricaPermessi?application=" + a.getName() + "&appType=" + a.getTipo_app() + "\"><img style=\"border:none;\" src=\"img/keys.gif\" ></img></a>");
						}

						if (gestServizi) {
							appAltri.append("<a href=\"CaricaServizi?application=" + a.getName() + "&appType=" + a.getTipo_app() + "\"><img style=\"border:none;\" src=\"img/services.png\" ></img></a>");
						}
			...
			out.append("<tr><th colspan=\"2\" nowrap=\"nowrap\" class=\"divTableTitle\" id=\"codiceEnte\">" + descrizioneSezione +" </th></tr>\n");
			...
			if (appAltri != null && appAltri.toString().length() > 1) {
				out.append("  <tr >\n");
				out.append("    <td width=\"1%\" nowrap=\"nowrap\">"+descAltriApp+ "</td>\n");
				out.append("    <td align=\"left\" >\n");
				out.append(appAltri);
				out.append("	 </td>\n");
				out.append("  </tr>\n");
