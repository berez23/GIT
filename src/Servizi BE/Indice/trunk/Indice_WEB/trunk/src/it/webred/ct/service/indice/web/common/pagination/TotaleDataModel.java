...
public class TotaleDataModel extends SerializableDataModel implements Serializable{

	private Map<String, SitOperationDTO> wrappedData = new HashMap<String, SitOperationDTO>();
	...
	public void walk(FacesContext context, DataVisitor visitor, Range range,Object argument) throws IOException {
		...
		if (detached) { // Is this serialized model
			...
			for (SitOperationDTO item : dataProvider.getTotaleByRange(
					firstRow, numberOfRows)) {

				System.out.println("Item ["+item.getCtrHash()+"]");
				wrappedKeys.add("" + item.getCtrHash());
				wrappedData.put("" + item.getCtrHash(),item);
				visitor.process(context, "" + item.getCtrHash(), argument);
			}
		}
	}
	...
	public int getRowCount() {
		rowCount = new Integer((int) dataProvider.getTotaleCount());
