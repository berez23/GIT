...
<%!public String scriviLayerMap(String ente, String webPath, String ana, String civ, String fgl, String par, String sub, String fogli, String particelle, String datasource, String intervento, String user) {
		Logger logger = Logger.getLogger("spprof.log");	
	
		String retVal = "";
		String param = null;
		String paramIntervento = null;
		if(ana != null)
			param = "ana-" + ana;
		if(civ != null)
			param = "civ-" + civ;
		...
		if(intervento != null)
			paramIntervento = "int-" + intervento;
		int i=1;
		...
		retVal += "#url of dbamp asj servlet to use for query \n"
				...
				"#url to use for information click \n"+ "infourl="+webPath+"/viewerjs_SpProf/extra/viewerjs/infoStrati?1=1\n"+

				"#url for 3d - optional\n"+ "threedimensionalurl=http://caleffi:8998/3d.jsp?1=1\n"+

				"#url for pictometry - optional \n"+ "pictometryurl=http://caleffi:8998/pictometry.jsp?1=1\n"+
		...
		if ("C618".equals(ente) || "C072".equals(ente) || "C208".equals(ente) || "C925".equals(ente) || "G170".equals(ente) || "G869".equals(ente) || "H410".equals(ente) || "H630".equals(ente) || "L380".equals(ente) || "L494".equals(ente)) {
			retVal += "layer.dbmapjs.name."+i+"=Foto 2007\n"
					+ "layer.dbmapjs.url."+i+"="+webPath+"/DbMAP_WMS_servlet_SpProf/dinamic/WMSservletEx?extraparam="+ente +"-0-"+datasource+"\n"
		...
		retVal += "layer.dbmapjs.name."+i+"=Interventi\n"
				+ "layer.dbmapjs.url."+i+"="+webPath+"/DbMAP_WMS_servlet_SpProf/dinamic/WMSservletEx?extraparam="+ente +"-0-"+datasource+"-"+user+"\n"
				+ "layer.dbmapjs.format."+i+"=image/gif\n"
				+ "layer.menu.name."+i+"=Interventi\n"
				+ "layer.dbmapjs.zoommin."+i+"=1\n"
				+ "layer.dbmapjs.zoomout."+i+"=15\n"
				+ "layer.dbmapjs.visible."+i+"=1\n"
				+ "layer.dbmapjs.opacity."+i+"=70\n"
				+ "layer.dbmapjs.layercolorlist."+i+"=205 92 92\n"
				+ "layer.dbmapjs.layerlist."+i+"=Interventi\n";
		i++;
		...
		try {
			...
			while (rs.next()) {
				...
				retVal += "layer.dbmapjs.name."+i+"="+ rs.getString("DESCRIZIONE") +"\n"
						+ "layer.dbmapjs.url."+i+"="+webPath+"/DbMAP_WMS_servlet_SpProf/dinamic/WMSservletEx?extraparam="+ente +"-"+ rs.getString("ID_CAT") + "-" + datasource +"\n"
						+ "layer.dbmapjs.format."+i+"=image/gif\n"
			...
			if(param!=null){
				retVal += "layer.dbmapjs.name."+i+"=Selezione\n"
						+ "layer.dbmapjs.url."+i+"="+webPath+"/DbMAP_WMS_servlet_SpProf/dinamic/WMSservletEx?extraparam="+ente +"-0"+ "-"+ datasource + "-" +param +"\n"
						+ "layer.dbmapjs.format."+i+"=image/gif\n"
			...
			if(paramIntervento!=null){
				retVal += "layer.dbmapjs.name."+i+"=Intervento\n"
						+ "layer.dbmapjs.url."+i+"="+webPath+"/DbMAP_WMS_servlet_SpProf/dinamic/WMSservletEx?extraparam="+ente +"-0"+ "-"+ datasource + "-" +paramIntervento +"\n"
						+ "layer.dbmapjs.format."+i+"=image/gif\n"
						+ "layer.menu.name."+i+"=Intervento\n"
						+ "layer.dbmapjs.zoommin."+i+"=1\n"
						+ "layer.dbmapjs.zoomout."+i+"=15\n"
						+ "layer.dbmapjs.visible."+i+"=1\n"
						+ "layer.dbmapjs.opacity."+i+"=80\n"
						+ "layer.dbmapjs.layercolorlist."+i+"=173 216 230,72 209 204,30 144 255,65 105 225\n"
						+ "layer.dbmapjs.layerlist."+i+"=Particelle Intervento,Fabbricati Intervento,Layer Intervento,Progetto Intervento\n";
				i++;
			}
