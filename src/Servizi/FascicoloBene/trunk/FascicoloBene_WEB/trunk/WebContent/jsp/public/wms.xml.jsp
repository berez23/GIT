...
String pkana=null;
String pkciv=null;
String codInventario=null;
String chiaveBene = null;
...
if(param.contains("inv")){
	chiaveBene=s[4];
	codInventario=s[4].split("@")[0];
  }
logger.debug("CODICE INVENTARIO-->>>"+codInventario+" - "+chiaveBene+" ("+param+")");
...
public String scriviLayerWms(String ente, Connection Conn, String Descrizione, String Tabella, String ShapeType, String SQLDescr, String IdCat, String PLDecodeDescr, String PLEncode, String IdField, long Storicizzato, String campoCodice)
{
  ...
  //logger.debug(" wms.xml.properties -->> "+ RetVal);
  return RetVal;
}
%>
...
%>
			...
			<% if(codInventario!=null){%>
			<Layer>
				<name>Inventario - Terreni</name>
				<InfoURL>../infoPart.jsp</InfoURL>
				<table>DM_B_MAPPALE_PART</table>
				<shape_field>SHAPE</shape_field>
				<%if(codInventario==null){%>
				<where>1=1 AND TIPO='T'</where>
				<%}else{%>
				<where>COD_INVENTARIO='<%=codInventario%>' AND TIPO='T'</where>
				<%}%>
				<shape_type>0</shape_type> 
				<text_label>CHIAVE||' - '||FOGLIO || ',  ' || PARTICELLA</text_label>
				<always_listed>Y</always_listed>
				<line_color>7</line_color>
				<line_thick>2</line_thick>
				<text_color>99</text_color>
				...
				<text_scale_min>1.5</text_scale_min>
				<point_style>1</point_style>
				<poly_fill_color>25</poly_fill_color>
				<poly_fill_style>1</poly_fill_style>
				<poly_fill_xor>N</poly_fill_xor>
			</Layer>
			<Layer>
				<name>Inventario - Mappali</name>
				<InfoURL>../infoPart.jsp</InfoURL>
				<table>DM_B_MAPPALE_PART</table>
				<shape_field>SHAPE</shape_field>
				<%if(codInventario==null){%>
				<where>1=1 AND TIPO='F'</where>
				<%}else{%>
				<where>COD_INVENTARIO='<%=codInventario%>' AND TIPO='F'</where>
				<%}%>
				<shape_type>0</shape_type> 
				<text_label>CHIAVE||' - '||FOGLIO || ',  ' || PARTICELLA</text_label>
				<always_listed>Y</always_listed>
				...
				<%if(!chiaveBene.equals(codInventario) && chiaveBene!=null){ %>
				<Layer>
					<name>Inventario - Bene</name>
					<InfoURL>../infoPart.jsp</InfoURL>
					<table>DM_B_MAPPALE_PART</table>
					<shape_field>SHAPE</shape_field>
					<where>CHIAVE='<%=chiaveBene%>'</where>
					<shape_type>0</shape_type> 
					<always_listed>Y</always_listed>
					<line_color>7</line_color>
					<line_thick>2</line_thick>
