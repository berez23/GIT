...
<%!public String scriviLayerMap(String ente, String webPath, String ana, String civ, String fgl, String par, String sub, String fogli, String particelle, String datasource, String inventario, String user) {
		Logger logger = Logger.getLogger("fascicolobene.log");
		
		String retVal = "";
		String param = null;
		String paramInventario = null;
		if(ana != null)
			param = "ana-" + ana;
		if(civ != null)
			param = "civ-" + civ;
		...
		if(inventario != null)
			paramInventario = "inv-" + inventario;
		int i=1;
		...
		retVal += "#url of dbamp asj servlet to use for query \n"
				...
				"#url to use for information click \n"+ "infourl="+webPath+"/viewerjs_FascicoloBene/extra/viewerjs/infoStrati?1=1\n"+

			    "#url for 3d - optional\n"+ "threedimensionalurl="+webPath+"/FascicoloBene_WEB/jsp/public/virtualearth.faces?1=1\n"+

				"#url for pictometry - optional \n"+ "pictometryurl="+webPath+"/FascicoloBene_WEB/jsp/public/streetview/streetview.faces?1=1\n"+
		...
		if ("C618".equals(ente) || "C072".equals(ente) || "C208".equals(ente) || "C925".equals(ente) || "G170".equals(ente) || "G869".equals(ente) || "H410".equals(ente) || "H630".equals(ente) || "L380".equals(ente) || "L494".equals(ente)) {
			retVal += "layer.dbmapjs.name."+i+"=Foto 2007\n"
					+ "layer.dbmapjs.url."+i+"="+webPath+"/DbMAP_WMS_servlet_FascicoloBene/dinamic/WMSservletEx?extraparam="+ente +"-0-"+datasource+"\n"
		...
		logger.debug("Param Inventario:"+paramInventario);
		if(paramInventario!=null){
			retVal += "layer.dbmapjs.name."+i+"=Inventario\n"
				+ "layer.dbmapjs.url."+i+"="+webPath+"/DbMAP_WMS_servlet_FascicoloBene/dinamic/WMSservletEx?extraparam="+ente +"-0"+ "-"+ datasource+"-" +paramInventario+"\n"
				+ "layer.dbmapjs.format."+i+"=image/gif\n"
				+ "layer.menu.name."+i+"=Inventario\n"
				+ "layer.dbmapjs.zoommin."+i+"=1\n"
				+ "layer.dbmapjs.zoomout."+i+"=15\n"
				+ "layer.dbmapjs.visible."+i+"=1\n"
				+ "layer.dbmapjs.opacity."+i+"=60\n"
				+ "layer.dbmapjs.layercolorlist."+i+"=0 100 0,205 92 92";
				if(inventario.contains("@"))
					retVal += ",0 0 255";
			retVal +="\n"
				+ "layer.dbmapjs.layerlist."+i+"=Inventario - Terreni,Inventario - Mappali";
				if(inventario.contains("@"))
					retVal += ",Inventario - Bene";
			retVal += "\n";
			i++;
		}
		...
		try {
			...
			while (rs.next()) {
				...
				retVal += "layer.dbmapjs.name."+i+"="+ rs.getString("DESCRIZIONE") +"\n"
						+ "layer.dbmapjs.url."+i+"="+webPath+"/DbMAP_WMS_servlet_FascicoloBene/dinamic/WMSservletEx?extraparam="+ente +"-"+ rs.getString("ID_CAT") + "-" + datasource +"\n"
						+ "layer.dbmapjs.format."+i+"=image/gif\n"
			...
			if(param!=null){
				retVal += "layer.dbmapjs.name."+i+"=Selezione\n"
						+ "layer.dbmapjs.url."+i+"="+webPath+"/DbMAP_WMS_servlet_FascicoloBene/dinamic/WMSservletEx?extraparam="+ente +"-0"+ "-"+ datasource + "-" +param +"\n"
						+ "layer.dbmapjs.format."+i+"=image/gif\n"
