...
public class DataModel extends SerializableDataModel implements Serializable {
	...
	public void walk(FacesContext context, DataVisitor visitor, Range range,
		...
		if (detached) { // Is this serialized model
			...
			for (DocfaTarReport item : dataProvider.getReportByRange(firstRow,
					numberOfRows)) {

				DocfaTarReportDTO dto = new DocfaTarReportDTO();
				if(item.getAnnotazioni() != null)
					item.setAnnotazioni(item.getAnnotazioni().replaceAll("@", ";"));
				dto.setDocfaTarReport(item);
				// gestione civici
				if (item.getCiviciDocfa() != null) {
					dto.getDocfaTarReport().setCiviciDocfa(item.getCiviciDocfa().replaceAll("@", ","));
				}
				...
				System.out.println("Item [" + item.getIdRep() + "]");
