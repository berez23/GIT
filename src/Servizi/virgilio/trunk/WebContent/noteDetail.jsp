...
<div class="section">
<h2>MUI - DETTAGLIO NOTA <b>${nota.numeroNotaTras }/${ nota.annoNota}</b> valida dal <jsp:setProperty name="FieldConverter" property="data"    value = "${nota.dataValiditaAtto}" />${FieldConverter.data}</h2>
<div class="section">
	...
	<tbody>
		...
		<tr class="a">
			<td align="center" colspan="20">
				<table>
					<tr>
						<td class="a">Tipo </td>
						<td><nota:logged nota="${nota}" propertyName="tipoNota">${nota.tipoNota }</nota:logged> </td>
						<td class="a">Registro particolare n.</td>
						<td></td>
						<td class="a">Data presentazione Atto</td>
						<td><nota:logged nota="${nota}" propertyName="dataPresAtto"><jsp:setProperty name="FieldConverter" property="data"    value = "${nota.dataPresAtto}" />${FieldConverter.data}</nota:logged> </td>
						<td class="a">Registrazione Diff.</td>
						<td><nota:logged nota="${nota}" propertyName="registrazioneDif"><c:choose><c:when test="${nota.registrazioneDif >0 }">SI</c:when><c:otherwise>NO</c:otherwise></c:choose></nota:logged> </td>
						<td class="a">Data in Diff.</td>
						<td><nota:logged nota="${nota}" propertyName="dataInDif"><jsp:setProperty name="FieldConverter" property="data"    value = "${nota.dataInDif}" />${FieldConverter.data}</nota:logged> </td>
						<td class="a">Nominativo Rog.</td>
						<td><nota:logged nota="${nota}" propertyName="cognomeNomeRog">${nota.cognomeNomeRog}</nota:logged> </td>
						<td class="a">Cod. Fisc Rog.</td>
						<td><nota:logged nota="${nota}" propertyName="codFiscRog">${nota.codFiscRog}</nota:logged> </td>
					</tr>
					<tr>
		...
		<c:forEach var="immobile"  varStatus="status" items="${fabbricatiNota}">
			...
			<tr class="b"><!--${immobile.iid}  -->
				...
				<td><nota:logged nota="${nota}" miDupFabbricatiInfo="${immobile}" propertyName="renditaEuro">${immobile.renditaEuro/100}</nota:logged> </td>
			<c:if test="${!empty immobile.codiceErroreImport}">
				...
				<tr onclick="popUp('formPostNoTemplate/importLogListNoTemplate.jsp?codiceEsito=true&iidImmobile=${immobile.iid}')" class="b">
					<td class="a"><c:choose><c:when test="${R0602.flagBloccante}"><img src="img/bloccante.gif"></c:when><c:otherwise><img src="img/nonbloccante.gif"></c:otherwise> </c:choose>Codice Esito</td>
		...
		<c:forEach var="titolarita"  varStatus="status" items="${titolaritaFavoreNota}">
			<c:if test="${status.first}">
				<tr class="a">
					<td class="a_title" align="center" colspan="20">SOGGETTI A FAVORE</td>
				</tr>
				<tr class="a">
