...
logger.debug("wms.xml.jsp param -->>> " + param);
...
if(param.contains("lot")){
	codLotto=s[4];
  }
logger.debug("CODICE ASI LOTTO-->>>"+codLotto+" - ("+param+")");
...
public String scriviLayerWms(String ente, Connection Conn, String Descrizione, String Tabella, String ShapeType, String SQLDescr, String IdCat, String PLDecodeDescr, String PLEncode, String IdField, long Storicizzato, String campoCodice, String infoCol, String infoDescr, String codLotto)
{
	
	Logger logger = Logger.getLogger("gitland.log");
	
	logger.info("GENERAZIONE LAYER DA wms.xml.jsp DENOMINATO: " + Descrizione + " COLLEGATO ALLA TABELLA: " + Tabella);
  ...
  try
  {
    ...
    while(RS.next())
    {
	  ...
	  if(Storicizzato==1){
		  if (Descrizione.equalsIgnoreCase("CENSIMENTO AZIENDALE")){
			  
			  if (codLotto != null)
			  	RetVal+="    <where>s." + IdField + " = "+ codLotto +" </where>";
			  else
				RetVal+="    <where> 1=1 </where>";
	    ...
	    if (Descrizione.equalsIgnoreCase("STA04: ZONE CENSUARIE")) {
	  	  ...
	  	  RetVal+="    <InfoURL>../infoPart.jsp</InfoURL>";
	    	  ...
	    	  RetVal+="    <text_visible>Y</text_visible>";    	
	    	  RetVal+="    <InfoURL>../infoPart.jsp</InfoURL>";
	    } else if (Descrizione.equalsIgnoreCase("CENSIMENTO AZIENDALE") ) {
	    	RetVal+="    <InfoURL>../infoPart.jsp</InfoURL>";
	    	RetVal+="    <text_label>s.ID_ASI</text_label>";
	    	RetVal+="    <line_thick>2</line_thick>";
	    	RetVal+="    <text_color>99</text_color>";
	    	RetVal+="    <text_height>12</text_height>";
	    	RetVal+="    <text_flags>0</text_flags>";
	    	RetVal+="    <text_font>SansSerif</text_font>";
	    	RetVal+="    <point_style>1</point_style>";
	    	RetVal+="    <poly_fill_xor>N</poly_fill_xor>";
  ...
  logger.info("FINE GENERAZIONE LAYER DA wms.xml.jsp DENOMINATO: " + Descrizione + " COLLEGATO ALLA TABELLA: " + Tabella);
...
<%=scriviLayerWms(ente, cnn, RS.getString("DESCRIZIONE"), RS.getString("TABELLA"), ShapeType, RS.getString("SQLDESCR"), RS.getString("ID_CAT"), RS.getString("PLDECODE_DESCR"), RS.getString("PLENCODE"), RS.getString("ID_FIELD"), RS.getLong("STORICIZZATO"), RS.getString("CODE_FIELD"), RS.getString("INFOCOL"), RS.getString("INFODESCR"), codLotto)%>
