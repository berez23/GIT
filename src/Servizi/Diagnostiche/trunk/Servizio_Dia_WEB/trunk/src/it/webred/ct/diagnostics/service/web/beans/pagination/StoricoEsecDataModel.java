...
public class StoricoEsecDataModel extends SerializableDataModel implements Serializable{

	private static final long serialVersionUID = 1L;

	private Map<Long, DiaTestataDTO> wrappedData = new HashMap<Long, DiaTestataDTO>();

	private List<Long> wrappedKeys = null;
	private boolean detached = false;
	private Long currentPk;

	private StoricoEsecDataProvider viewDataProvider;
	...
	public void walk(FacesContext context, DataVisitor visitor, Range range,Object argument) throws IOException {
		...
		if (detached) { // Is this serialized model
			...
			for (DiaTestataDTO item : viewDataProvider.getVisualizzaByRange(firstRow, numberOfRows)) {

				//System.out.println("Item ["+item[0]+"]");
				wrappedKeys.add(item.getObjTestata().getId());
				wrappedData.put(item.getObjTestata().getId(),item);
				visitor.process(context,item.getObjTestata().getId(), argument);
			}
		}
	}
	...
	public StoricoEsecDataProvider getViewDataProvider() {
		return viewDataProvider;
	}

	public void setViewDataProvider(StoricoEsecDataProvider viewDataProvider) {
