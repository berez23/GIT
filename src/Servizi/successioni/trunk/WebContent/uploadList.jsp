...
<div class="section">
<h2>SUCCESSIONI - FORNITURE CARICATE<c:if test="${!empty param.toExport }"> DA ESPORTARE</c:if>
</h2>
<div class="section">
<p>file caricati&nbsp;&nbsp;&nbsp;
<c:choose>
...
<display:table id="caricamento" export="false"
	...
	<c:choose>
		<c:when test="${empty param.toLogInfo }">
			<c:if test="${false && empty param.toExport }">
				<security:lock role="succ-adm,succ-supusr">
					...
					<display:column title="com.ICI CIAOO">
						<c:if
							test="${caricamento.miDupFornituraInfo.note == caricamento.miVwNoteSummary.totale }">
							<c:if
								...
								<c:if test="${empty converters[caricamento.iid]  }">
									<a
										...
										role="succ-adm,succ-supusr">genera</security:lock> </a>
								</c:if>
								<c:if test="${!empty converters[caricamento.iid]  }">
									<c:set var="REFRESH_TIMEOUT" scope="request"
										value="${REFRESH_TIMEOUT + 10}" />
						  		in generazione (${converters[caricamento.iid].rowCount}/${caricamento.miVwTitolariSummary.totale })
							</c:if>
							</c:if>
							<c:if
								test="${!empty caricamento.miVwComunicazioneSummary.totale  }">
			...
			<security:lock role="succ-adm,succ-supusr">
				<display:column title="integrazione">
					<c:if test="${caricamento.miDupFornituraInfo.note == caricamento.miVwNoteSummary.totale }">
						...
						<c:if test="${empty integrators[caricamento.iid]  }">
							<c:if test="${!caricamento.integrata}">
								<a href="formPostNoTemplate/integrazioneResult.jsp?iidFornitura=${caricamento.iid }">integra</a>
							</c:if>
							<c:if test="${caricamento.integrata}">
								integrata
							</c:if>
				...
				<display:column title="integrazione fabbr.">
					<c:if test="${caricamento.miDupFornituraInfo.note == caricamento.miVwNoteSummary.totale }">
						<c:if test="${!empty integratorsFabbr[caricamento.iid]  }">
							<c:set var="REFRESH_TIMEOUT" scope="request" value="${REFRESH_TIMEOUT + 10}" />
					  		in generazione (${integratorsFabbr[caricamento.iid].rowCount}/${caricamento.miVwTitolariSummary.totale })
						</c:if>
						<c:if test="${empty integratorsFabbr[caricamento.iid]  }">
							<c:if test="${!caricamento.integrataFabbr}">
								<a href="formPostNoTemplate/integrazioneFabbrResult.jsp?iidFornitura=${caricamento.iid }">integra fabbricati</a>
							</c:if>
							<c:if test="${caricamento.integrataFabbr}">
								fabbricati integrati
							</c:if>							
						</c:if>
					</c:if>
				</display:column>
				...
				<c:if test="${!empty param.toExport }">
					<display:column title="esporta">
						<c:if test="${caricamento.miDupFornituraInfo.note == caricamento.miVwNoteSummary.totale }">
							<!--  <a href="formPostNoTemplate/uploadExportNoTemplate?iidFornitura=${caricamento.iid }">esporta</a> -->
							<a href="uploadFromDB.jsp?iidFornitura=${caricamento.iid }&esporta=S&coniugato=S">Comunicazioni coniugati</a>
						</c:if>
					</display:column>
					<display:column title="esporta">
						<c:if test="${caricamento.miDupFornituraInfo.note == caricamento.miVwNoteSummary.totale }">
							<!--  <a href="formPostNoTemplate/uploadExportNoTemplate?iidFornitura=${caricamento.iid }">esporta</a> -->
							<a href="uploadFromDB.jsp?iidFornitura=${caricamento.iid }&esporta=S&coniugato=N">Comunicazioni celibi</a>
						</c:if>
					</display:column>
				</c:if>
				<c:if test="${empty param.toExport }">
				...
				<display:column title="Dap">
					<c:choose>
						<c:when test="${miConsDaps[caricamento.iid].fullyEvaluated}">
							...
							<a
								href="javascript:eliminaMiConsDap(${caricamento.iid})">elimina</a>
							<p>
								<a href="formPostNoTemplate/noteSearch.jsp?miDupFornitura.iid=${caricamento.iid }&flagSkipped=Y">${miConsDaps[caricamento.iid].dapNA} dap non applicabile</a>,<a href="formPostNoTemplate/noteSearch.jsp?miDupFornitura.iid=${caricamento.iid }&flagSkipped=N&flagDapDiritto=Y"> ${miConsDaps[caricamento.iid].dapY} con detrazione</a>,<a href="formPostNoTemplate/noteSearch.jsp?miDupFornitura.iid=${caricamento.iid }&flagSkipped=N&flagDapDiritto=N"> ${miConsDaps[caricamento.iid].dapN} senza detrazione</a>							
							</p>	
						</c:when>
						<c:otherwise>
							<c:choose>
								...
								<c:otherwise>
									<a
										href="formPostNoTemplate/dapEval.jsp?iidFornitura=${caricamento.iid }">genera
