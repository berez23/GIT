...
public class MuiFornituraParser implements MuiHttpConstants {
	...
	public void parse(boolean xmlOriginated) throws IOException {
		try {
			...
			if (rt1 != 1) {
				throw new MuiInvalidInputDataException(
						"Il Formato del File e' errato, il file non comincia con l'header della fornitura note!");
			}
			...
			try {
				...
				Logger.log().error("Il Formato del File e' errato ",
						_currentline, t);
				throw new MuiInvalidInputDataException(
						"Il Formato del File e' errato, errore durante la lettura dell'header della fornitura note!");
			}
			...
			Query query = session
					.createQuery("select fornitura from MiDupFornitura as fornitura  where"
							+ " (fornitura.dataInizialeDate <= :dataInizialeDate AND fornitura.dataFinaleDate > :dataInizialeDate) or "
							+ " (fornitura.dataInizialeDate <= :dataFinaleDate AND fornitura.dataFinaleDate >= :dataFinaleDate) or "
							+ " (fornitura.dataInizialeDate >= :dataInizialeDate AND fornitura.dataFinaleDate <= :dataFinaleDate) ");
			query.setDate("dataInizialeDate", dateParser.parse(miDupFornitura
					.getDataIniziale()));
			query.setDate("dataFinaleDate", dateParser.parse(miDupFornitura
					.getDataFinale()));
			...
			for (Iterator it = query.iterate(); it.hasNext();) {
				...
				Logger
						...
						.info(this.getClass().getName(),
								"Il File ha intervalli temporali che si sovrappongono a caricamenti gia' effettuati! "+
								overlapping);
			}
			/*
			 * if(true){ return; }
			...
			if (overlapping != null){
				if(!xmlOriginated) {
					Logger
					.log()
					.error(this.getClass().getName(),"Il File ha intervalli temporali che si sovrappongono a caricamenti gia' effettuati! - "+
							_currentline);
					throw new MuiInvalidInputDataException(
					"Il File ha intervalli temporali che si sovrappongono a caricamenti gia' effettuati! errore durante la lettura dell'header della fornitura note!");					
				}
				else{
					Logger
					.log()
					.error(this.getClass().getName(),"Il File ha intervalli temporali che si sovrappongono a caricamenti gia' effettuati - condizione consentita perche' file originale in xml"+
							_currentline);
					// recordParser.store();
			...
			Logger.log().error(this.getClass().getName(),"errore durante la valutazione del record - "+
					_currentline, iae);
			throw new MuiException("errore durante la valutazione del record",
					_currentline, iae);
	...
	private void handlePartialLoading(Session session) throws ParseException {
		Query query;
		query = session
				.createQuery("select fornitura from MiDupFornitura as fornitura  where "
						+ " fornitura.dataInizialeDate = :dataInizialeDate AND fornitura.dataFinaleDate = :dataFinaleDate");
		...
		if(partiallyLoaded != null){
			...
			.log()
			.info(this.getClass().getName(),
					"Il File e' parte di un caricamento multiplo "+
					partiallyLoaded);
			...
			for (int i = 0; i < loaded.length; i++) {
				if (miDupFornitura.getFileLoadedNotParsed().equals(loaded[i])){
					Logger
					.log()
					.error(this.getClass().getName(),"Il File n."+loaded[i]+"/"+partiallyLoaded.getFileTotNotParsed()+" per il periodo indicato è già stato caricato ! ");
					throw new MuiInvalidInputDataException(
							"Il File n."+loaded[i]+"/"+partiallyLoaded.getFileTotNotParsed()+" per il periodo indicato è già stato caricato ! ");					
				}
			}
	...
	public void parseLastLine(String line) throws MuiInvalidInputDataException,IOException {
		MuiFornituraRecordParser recordParser;
		_currentline = line;
		if (_currentline == null || _currentline.trim().length()==0) {
			throw new MuiInvalidInputDataException(
			"Il Formato del File e' errato, il file non termina con il footer della fornitura note!");
		}
		int rt1 = evalRecordType();
		if (rt1 != 9) {
			throw new MuiInvalidInputDataException(
		...
		try {
			...
			Logger.log().error("Il Formato del File e' errato ",
					_currentline, t);
			throw new MuiInvalidInputDataException(
					"Il Formato del File e' errato, errore durante la lettura del footer della fornitura note!");
		}
		...
		if(miDupFornitura.getMiDupFornituraInfo() != null){
			Logger
			.log()
			.info(this.getClass().getName(),"caricamento su stesso periodo, aggiorno fornituraInfo ");
	...
	public void parseNotes() {
		...
		try {
			while ((_currentline = lnr.readLine()) != null) {
				...
				try {
					...
					if (recordParser != null) {
						...
						recordParser.parse(_currentline);
						setPojoProperty(recordParser.getRecord(),
								"miDupFornitura", miDupFornitura);
						if (rt == NOTA_RECORD_TYPE) {
							...
							if (miDupNotaTras != null) {
								for (Iterator iter = indirizziNota.iterator(); iter
										.hasNext();) {
									MiDupIndirizziSog element = (MiDupIndirizziSog) iter
											.next();
									try {
										element.setMiDupSoggetti(soggettiNota
												.get(Integer.valueOf(element
														.getIdSoggettoNota())));
									} catch (NumberFormatException nfe) {
									}
								}
								for (Iterator iter = fabbricatiIdentificaNota
										.iterator(); iter.hasNext();) {
									MiDupFabbricatiIdentifica element = (MiDupFabbricatiIdentifica) iter
											.next();
									try {
										element
												.setMiDupFabbricatiInfo(fabbricatiInfoNota
														.get(Integer
																.valueOf(element
																		.getIdImmobile())));
									} catch (NumberFormatException nfe) {
								...
								for (Iterator iter = titolaritaNota.iterator(); iter
										.hasNext();) {
									MiDupTitolarita element = (MiDupTitolarita) iter
											.next();
									try {
										element
												.setMiDupFabbricatiInfo(fabbricatiInfoNota
									...
									try {
										element.setMiDupTerreniInfo(terreniNota
												.get(Integer.valueOf(element
														.getIdImmobile())));
									} catch (NumberFormatException nfe) {
									}
							...
							miDupNotaTras = (MiDupNotaTras) recordParser
									.getRecord();
							...
							if (rt == SOGGETTO_RECORD_TYPE) {
								...
								parseFabbricatoInfo(recordParser,
										fabbricatiInfoNota);
							} else if (rt == IMMOBILEIDENTIFICA_RECORD_TYPE) {
								parseFabbricatoIdentifica(recordParser,
										fabbricatiIdentificaNota);
					...
					Logger.log().error(this.getClass().getName(),
							"errore durante la valutazione del record - "+
							_currentline, t);
					continue;
				}
			}
			...
			try {
				Logger.log().error(this.getClass().getName(),
						"rollback caricamento per eccezione Hibernate - "+
						e, e);
				tx.rollback();
			...
			try{
				...
				Logger.log().error(this.getClass().getName(),
						"errore durante l'annullamento automatico del caricamento della fornitura",
						 e);
			}
			try {
				HibernateUtil.closeSession();
			...
			Logger.log().error(this.getClass().getName(),
					"rollback caricamento per eccezione I/O - "+
					e, e);
			try {
				tx.rollback();
			} catch (HibernateException e1) {
			...
			try {
				Logger.log().error(this.getClass().getName(),
						"rollback caricamento per eccezione - "+
						e, e);
				tx.rollback();
			} catch (HibernateException e1) {
			}
	...
	protected void parseIndirizzo(MuiFornituraRecordParser recordParser,
			List<MiDupIndirizziSog> indirizziNota) {
		MiDupIndirizziSog indirizzo = (MiDupIndirizziSog) recordParser
				.getRecord();
	...
	private void parseTerreno(MuiFornituraRecordParser recordParser,
			Map<Integer, MiDupTerreniInfo> terreniNota) {
		MiDupTerreniInfo terreniInfo = (MiDupTerreniInfo) recordParser
				.getRecord();
		if (recordParser.check(terreniInfo)) {
			try {
				terreniNota.put(Integer.valueOf(terreniInfo.getIdImmobile()),
						terreniInfo);
			} catch (NumberFormatException nfe) {
	...
	protected void parseTitolarita(MuiFornituraRecordParser recordParser,
	...
	protected void parseFabbricatoIdentifica(
			MuiFornituraRecordParser recordParser,
			List<MiDupFabbricatiIdentifica> fabbricatiIdentificaNota) {
		MiDupFabbricatiIdentifica fabbricatoIdentifica = (MiDupFabbricatiIdentifica) recordParser
				.getRecord();
	...
	protected void parseFabbricatoInfo(MuiFornituraRecordParser recordParser,
			Map<Integer, MiDupFabbricatiInfo> fabbricatiInfoNota) {
		MiDupFabbricatiInfo fabbricatoInfo = (MiDupFabbricatiInfo) recordParser
				.getRecord();
		if (recordParser.check(fabbricatoInfo)) {
			try {
				fabbricatiInfoNota.put(Integer.valueOf(fabbricatoInfo
						.getIdImmobile()), fabbricatoInfo);
			} catch (NumberFormatException nfe) {
	...
	protected void parseSoggetto(MuiFornituraRecordParser recordParser,
		...
		if (recordParser.check(soggetto)) {
			try {
				soggettiNota.put(Integer.valueOf(soggetto.getIdSoggettoNota()),
						soggetto);
			} catch (NumberFormatException nfe) {
	...
	private MuiFornituraRecordParser getRecordParserForRecordType(int rt) {
		...
		if (recordParser == null) {
			try {
				recordParser = (MuiFornituraRecordParser) Class.forName(
						"it.webred.mui.input.parsers.MuiFornituraRecordTipo"
								+ rt + "Parser").newInstance();
				_parsers.put(rt, recordParser);
				MuiFornituraRecordChecker recordChecker = (MuiFornituraRecordChecker) Class
						.forName(
								"it.webred.mui.input.parsers.MuiFornituraRecordTipo"
										+ rt + "Checker").newInstance();
	...
	private Long getIdNota() {
		...
		try {
			...
			Logger.log().error(this.getClass().getName(),
					"errore durante la valutazione idNota del record = \""+ 
					_currentline+"\" idNota=\""+rf.field+"\"", e);
			throw e;
		}
	...
	public static RowField getNextField(String sline, RowField rf, char delimiter) {
		// If Added by MaX 17/12/2007 - Perché in alcuni casi si arriva qui con sline == NULL e andava in errore
		if (sline == null) {
			return null;
		}
	...
	public static void setPojoProperty(Object pojo, String fieldName,
			Object fieldValue) {
	...
	public static void setPojoProperty(Object pojo, String fieldName, Object fieldValue, Class pojoClass) {
		String fieldSetterName = "set" + fieldName.toUpperCase().charAt(0)
				+ (fieldName.length() > 1 ? fieldName.substring(1) : "");
		try {
			Method method = getPojoMethod(fieldValue, fieldSetterName,
					pojoClass);
	...
	private static Method getPojoMethod(Object fieldValue, String methodName,
			Class pojoClass) throws NoSuchMethodException {
	...
	public String getLastLine(java.io.File file) throws IOException{
		
	/*	 BufferedReader in = new BufferedReader(new FileReader( file )) ;
		 String ltr, str=null;
		 
         while ((ltr = in.readLine()) != null) 
         	{str=ltr;}
