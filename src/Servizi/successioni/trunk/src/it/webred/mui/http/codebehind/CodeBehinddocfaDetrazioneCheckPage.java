...
public class CodeBehinddocfaDetrazioneCheckPage extends AbstractPage {
		...
		try { 
			...
			while (ogs.hasNext()) {
				...
				for (Iterator<CodiceViaCivico> iter = oggetto.getCodiceViaCivicos(); iter.hasNext(); i++) {
					...
					msg += getVia(cvc.getCodiceVia()) + "(codice via ="
	...
	private String getVia(String codice) throws Exception {
		String res = viario.decodeVia(codice);
		if (res == null) {
			return "";
		} else {
			return res;
