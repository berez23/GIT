...
%>
	...
	<table>
		...
		<tr><td> - EDITING (optional)[I/U/D]: </td><td> [<i> <%=editing%> </i>]</td></tr>
...
<%
					...
					{%>
						<td><input type="TEXT" name="field_<%=RsMd.getColumnName(i)%>" size=<%=(RsMd.getColumnDisplaySize(i)<=80?RsMd.getColumnDisplaySize(i):80)%> value="<%=getColumnString(request,Rs,RsMd,i)%>" <%=(editing.equalsIgnoreCase("D")?"DISABLED":"")%>></td>
					<%}%>
				</tr>
				<%
				}
