...
public class Flusso750RuoloDataModel extends SerializableDataModel implements
		Serializable {

	private Map<Long, InfoRuoloDTO> wrappedData = new HashMap<Long, InfoRuoloDTO>();
	...
	private Flusso750RuoloDataProvider dataProvider;
	...
	public void walk(FacesContext context, DataVisitor visitor, Range range,
		...
		if (detached) { // Is this serialized model
			...
			for (InfoRuoloDTO item : dataProvider.getDataByRange(
					firstRow, numberOfRows, criteria)) {
				
				wrappedKeys.add(item.getInfoRuolo().getId());
				wrappedData.put(item.getInfoRuolo().getId(),
						item);
				visitor.process(context, item.getInfoRuolo().getId(), argument);
			}
		}
	}
	...
	public Object getRowData() {
		if (currentPk == null) {
			...
			InfoRuoloDTO ret = wrappedData.get(currentPk);
			if (ret == null) {
	...
	public Flusso750RuoloDataProvider getDataProvider() {
		return dataProvider;
	}

	public void setDataProvider(Flusso750RuoloDataProvider dataProvider) {
	...
	public void _resetData() {
		...
		wrappedData = new HashMap<Long, InfoRuoloDTO>();
		wrappedKeys = null;
	}
