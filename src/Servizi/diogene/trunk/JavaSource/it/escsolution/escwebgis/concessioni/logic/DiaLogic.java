 ...
 *         Comments
 */
public class DiaLogic extends EscLogic
{
	private String appoggioDataSource;

	public DiaLogic(EnvUtente eu)
	{
		super(eu);
	...
	public final static String LISTA = "LISTA_DIA";
	public final static String DIA = "DIA";
	public static final String DIACONC = DiaLogic.class.getName() + "@DIACONC";
	...
	public Hashtable mCaricareLista(Vector listaPar, DiaFinder finder)
		throws Exception
	{
			...
			for (int i = 0; i <= 1; i++)
			{
				...
				if (i == 1)
				{
					while (rs.next())
					{
						// campi della lista
						Dia dia = new Dia();
						dia.setAnno(rs.getString("ANNOPG"));
						dia.setNProtocollo(rs.getString("NUMEROPG"));
						if (rs.getString("AREAPG") == null)
							dia.setArea("-");
						else
							dia.setArea(rs.getString("AREAPG"));
						dia.setDenominazione(rs.getString("DENOMINAZIONE"));
						if (rs.getString("OGGETTO").length()>40)
							dia.setOggetto(rs.getString("OGGETTO").substring(0,40)+"...");
						else
							dia.setOggetto(rs.getString("OGGETTO"));
