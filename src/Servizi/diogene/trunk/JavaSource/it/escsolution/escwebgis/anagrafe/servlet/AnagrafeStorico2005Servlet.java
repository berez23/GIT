...
public class AnagrafeStorico2005Servlet extends EscServlet implements EscService {

	private String recordScelto;
	public static final String NOMEFINDER = "FINDER36";
	private AnagrafeStorico2005Finder finder = null;
	...
	private void mCaricareFormRicerca(HttpServletRequest request, HttpServletResponse response) throws Exception{
				...
				*/
				...
				nav.chiamataRicerca();
				this.chiamaPagina(request,response,"anagrafe/anaStorico2005Frame.jsp", nav);
	...
	private void mCaricareLista(HttpServletRequest request,HttpServletResponse response)throws Exception{
		...
		if (sessione.getAttribute(NOMEFINDER)!= null){
			if (((Object)sessione.getAttribute(NOMEFINDER)).getClass() == new AnagrafeStorico2005Finder().getClass()){
				finder = (AnagrafeStorico2005Finder)sessione.getAttribute(NOMEFINDER);
			}
			else
				finder = null;
		}
		...
		finder = (AnagrafeStorico2005Finder)gestireMultiPagina(finder,request);

		AnagrafeStorico2005Logic logic = new AnagrafeStorico2005Logic(this.getEnvUtente(request));
		Hashtable ht = logic.mCaricareLista(vettoreRicerca,finder);


	    Vector vct_lista= (Vector)ht.get(AnagrafeStorico2005Logic.LISTA);
		finder = (AnagrafeStorico2005Finder)ht.get(AnagrafeStorico2005Logic.FINDER);
		sessione.setAttribute(AnagrafeStorico2005Logic.LISTA, vct_lista);
	...
	private void mCaricareDettaglio(HttpServletRequest request,HttpServletResponse response)throws Exception{
				...
				AnagrafeStorico2005Finder finder = null;
				...
				gestioneMultiRecord(request,response,azione,finder,sessione,AnagrafeStorico2005Logic.LISTA,(Vector)sessione.getAttribute(AnagrafeStorico2005Logic.LISTA),NOMEFINDER);
				if (azione.equals("")){
				...
				Vector vct_dettStorico= (Vector)ht.get(AnagrafeStorico2005Logic.ANAGRAFE_STORICO);

				sessione.setAttribute(AnagrafeStorico2005Logic.ANAGRAFE_STORICO, vct_dettStorico);
	...
	public EscFinder getNewFinder(){
		return new AnagrafeStorico2005Finder();
	}
	public String getChiaveOggetto(EscFinder finder, Vector listaOggetti,int recordSuccessivo){
		return ((AnagrafeStorico)listaOggetti.get(recordSuccessivo)).getChiave();
	}
