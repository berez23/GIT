...
<html>
	...
	<%if(ST.equals("3") && "1".equals(request.getParameter("NONAV"))){%>
		<frameset rows="0,*" border="0">
			...
			<frame name="bodyPage" src="<%= it.escsolution.escwebgis.common.EscServlet.URL_PATH %>/f24/F24AnnullamentoDetail.jsp" noresize class="frame">
		</frameset>
	<%}else{%>
		<%if(ST.equals("1")){%>
		...
		<%}else if(ST.equals("2")){%>
			<frame name="bodyPage" src="<%= it.escsolution.escwebgis.common.EscServlet.URL_PATH %>/f24/F24AnnullamentoList.jsp" noresize class="frame">
		<%}else{%>
			<frame name="bodyPage" src="<%= it.escsolution.escwebgis.common.EscServlet.URL_PATH %>/f24/F24AnnullamentoDetail.jsp" noresize class="frame">
		<%}%>
