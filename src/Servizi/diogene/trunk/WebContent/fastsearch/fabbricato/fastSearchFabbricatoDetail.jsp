...
<%  
	...
	HashMap<String, IndiceBean> oggettiMap = (HashMap) session.getAttribute("fabbricatiMap");
	HashMap<String, Fonte> fonteDescr = (HashMap) session.getAttribute("fonteDescr");
	FabbricatoTotaleBean fabb = (FabbricatoTotaleBean) session.getAttribute("fabbIndiceUnico");
	...
	<tr>
	 <td class="extWinTDTitle" width="50%">
 			<span class="extWinTXTTitle">
 	  			<%
 				String descr1 =  " SEZ.: " + fabb.getSezione()+ " F.: "+ fabb.getFoglio() + " P.: " + fabb.getParticella() ;
 			...
 			<span class="extWinTXTData">
 			  <% if (ib != null) { %>
		 			  <span class="TXTmainLabel">
 							...
 							onclick="eseguiCrossLinkInd('<%=ib.getIdUnico()%>', '<%=ib.getFonteId()%>', 5, '<%=descr1 %>')"
