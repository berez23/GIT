...
<%  
	...
	ViaTotaleBean via = (ViaTotaleBean) session.getAttribute("viaIndiceUnico");
	...
	<tr>
	 <td class="extWinTDTitle" width="50%">
 			<span class="extWinTXTTitle">
 	  			Indirizzo: <%=via.getSedime() %> &nbsp; <%=via.getIndirizzo() %> 
 	  		 </span>
 	  	</td>
	</tr>
</table>
 			  ...
 			  <% if (ib != null) { %>
		 			  <span class="TXTmainLabel">
 							<img src="../../images/<%=img%>" 
 							  	 ...
 							  	 onclick="eseguiCrossLinkInd('<%=ib.getIdUnico()%>', '<%=ib.getFonteId()%>', 2, '<%=via.getSedime() %> &nbsp; <%=via.getIndirizzo() %>')"
