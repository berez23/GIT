...
<% it.escsolution.escwebgis.dup.bean.DupElementoListaNoteFinder finder = (it.escsolution.escwebgis.dup.bean.DupElementoListaNoteFinder)sessione.getAttribute("FINDER34"); %>
<%int js_back = 1;
...
function vai(codice, record_cliccato, isPopUp)
{
	try
	{
		...
		if (isPopUp)
		{
			targ = apriPopUp(34000+record_cliccato);
...
function apriPopUp(index)
{
	if (popUpAperte[index])
	{
		...
		{
			popUpAperte[index] = window.open("", "PopUpNoteTerrDetail" + index, "width=900,height=480,status=yes,resizable=yes");
