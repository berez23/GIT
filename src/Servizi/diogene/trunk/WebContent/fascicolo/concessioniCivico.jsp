...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jstl/fmt" %>
   <fieldset>
  	<legend>CONCESSIONI AL CIVICO</legend>
	...
	<c:forEach var="conc" items="${DATI_FASCICOLO}">
	     <tr onclick="dettaglioConcessioni('<c:out value="${conc.id}"/>')">
			...
			<td class="extWinTDData" style='cursor: pointer;'>
				<c:forEach var="indirizzo" items="${conc.listaSoggetti}">
					<span class="extWinTXTData"><c:out value="${indirizzo.titolo} ${indirizzo.datiAnag}"/></span>
				</c:forEach>
			</td>
			...
			<td class="extWinTDData" style='cursor: pointer;'>
				<c:forEach var="indirizzo" items="${conc.listaIndirizzi}">
					<span class="extWinTXTData"><c:out value="${indirizzo.indirizzo} ${indirizzo.civico}"/></span>
				</c:forEach>
			</td>
