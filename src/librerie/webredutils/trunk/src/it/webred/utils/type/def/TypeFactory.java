...
import it.webred.utils.type.ArrayOfByte;
import it.webred.utils.type.ComplexParam;
import it.webred.utils.type.Constant;
import it.webred.utils.type.ScriptBeanShell;
import it.webred.utils.type.Variable;
import it.webred.utils.type.bean.ComplexParamP;
import it.webred.utils.DateFormat;
import it.webred.utils.HashMapUtils;
import it.webred.utils.WebredUtilsException;
...
{
	private static HashMap<Class, TypeConstructor> dataTypeList = new HashMap<Class,TypeConstructor>();
	...
	{	
		...
		dataTypeList.put(it.webred.utils.type.ComplexParam.class,(tf.new ComplexParamConstructor()));
		dataTypeList.put(it.webred.utils.type.ScriptBeanShell.class,(tf.new ScriptBeanShellConstructor()));
		dataTypeList.put(it.webred.utils.type.Constant.class,(tf.new ConstantConstructor()));
		dataTypeList.put(it.webred.utils.type.Variable.class,(tf.new VariableConstructor()));
	 ...
	 * @throws Exception
	 */
	private static TypeConstructor getConstructor(String type) throws WebredUtilsException 
	{
		try {
			
		Object constructor = dataTypeList.get(Class.forName(type));
		if (constructor==null) {
			constructor = dataTypeList.get(Class.forName(type).getSuperclass());
		} 
		if (constructor==null) 
			throw new WebredUtilsException("Costruttore non reperito per il tipo di dato " + type);
		...
		return tc;
			...
			throw new WebredUtilsException("Problemi con il tipo di dato " + type);
		}
	...
	public static List<String> getCompatibleType(String className) throws Exception 
	{
		 ...
		 */
		public static Object getType(Object obj, String type) throws WebredUtilsException 
		{
			if (type==null || obj==null)
				if (type.indexOf("it.webred.rulengine.dwh.def")==-1) // per i miei tipi so come comportarmi
					return null;
				...
				if(!constructor.haveStringConstructor() && obj instanceof java.lang.String)
					throw new WebredUtilsException("Si sta cercando di costruire attraverso una stringa un tipo di dato " + type);
	...
	public abstract class TypeConstructor {
		 ...
		 * @throws Exception
		 */
		public abstract Object construct(Object obj) throws WebredUtilsException ;
	...
	public class DateConstructor extends TypeConstructor {
		...
		public Object construct(Object obj) throws WebredUtilsException
		{
			if (!(obj instanceof java.sql.Date))
			{
				...
				if (d==null && obj!=null)
					throw new WebredUtilsException("Impossibile convertire il parametro data secondo il formato dd-MM-yyyy");
				return d;
	...
	public class TimestampConstructor extends TypeConstructor {
		...
		public Object construct(Object obj) throws WebredUtilsException
		{
			...
			if (errato && d==null)
				...
				if (errato)	
					throw new WebredUtilsException("Impossibile convertire il parametro data / ora secondo uno dei formati previsti - " + (String) obj);
		...
		public Object construct(Object obj) throws WebredUtilsException
		{
			if (!(obj instanceof ScriptBeanShell))   
			{
				...
				catch (Exception e)
				{
					...
					throw new WebredUtilsException("Creazione del tipo di dato ScriptBeanShell non ruscita - possibili errori di configurazione");
				}
			} else {
				return obj;
			}
	...
	public class ComplexParamConstructor extends TypeConstructor implements DeclarativeTypeAccessor{
		...
		public Object construct(Object obj) throws WebredUtilsException
		{
			if (!(obj instanceof ComplexParam))   
			{
				String config = (String) obj; 
				try
				{
				...
				{
					log.error("Errori ComplexParamConstructor",e);
					throw new WebredUtilsException("Creazione del tipo di dato ComplexParam non ruscita - possibili errori di configurazione");
				}
			} else {
				return obj;
			}
		...
		{
			this.var = var;
			ComplexParam cp = (ComplexParam)this.construct(obj);
...
//				    System.out.println(TypeFactory.getType("31-12-2004",item.getKey(),null));
